...
# The standing apply-discipline directive both VP clients advertise to a
# repo-mutation session (the CLI client in ``_build_cli_prompt``, the SDK
# client appended to the objective). ONE shared constant so the two lanes can
# never drift. It exists so the checkpoint gets written in the first place —
# without it, an agent runs its ``apply_*.py`` directly and a finalize crash
# orphans the work with no durable marker.
...
def apply_discipline_requested(
    payload: Optional[dict[str, Any]],
    constraints: Optional[dict[str, Any]] = None,
) -> bool:
    """True when the mission asks for repo mutation, wherever the flag lives.

    Real mission payloads carry the repo-mutation signal in one of two
    shapes: at the payload top level (``repo_mutation_allowed`` /
    ``workflow_kind`` / ``mission_type``) or nested under
    ``payload["constraints"]`` (the dispatcher's `_extract_target_paths`
    vocabulary). Check both so the discipline section reaches every
    repo-mutation mission regardless of which producer built the payload.
    """
    from universal_agent.codebase_policy import repo_mutation_requested

    if repo_mutation_requested(payload):
        return True
    if constraints is None and isinstance(payload, dict):
        raw = payload.get("constraints")
        constraints = raw if isinstance(raw, dict) else None
    return repo_mutation_requested(constraints)
