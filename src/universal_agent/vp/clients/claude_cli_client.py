...
def _build_cli_prompt(
    ...
    *,
    ...
    if apply_discipline_requested(payload):
        parts.append(APPLY_DISCIPLINE_SECTION)
    ...
    if workspace_dir is not None:
        try:
            ...
            if has_validated_apply(workspace_dir):
                ...
                # No validated checkpoint — but if the prior attempt left
                # apply-script(s) in the workspace, the apply MAY have run
                # outside the blessed runner, leaving no durable marker either
                # way. "No checkpoint" must not be read as "apply never ran":
                # direct the retry to verify repo state before any re-apply.
                leftover = sorted(p.name for p in workspace_dir.glob("apply*.py"))
                if leftover:
                    parts.append(
                        "\n## CAUTION: prior attempt left apply script(s) — verify before re-applying\n\n"
                        f"The previous attempt left {', '.join(f'`{n}`' for n in leftover)} "
                        f"in this workspace but recorded NO validated checkpoint, so it is "
                        f"UNKNOWN whether the script(s) already ran. Re-running an "
                        f"anchor-consuming apply script against an already-patched repo is "
                        f"destructive. Before running ANY apply script: check the target "
                        f"repo with ``git status`` / ``git diff`` for the edits it would "
                        f"make. If the edits are already present, DO NOT re-run — commit "
                        f"and resume from push/PR instead. If you do need to apply, run it "
                        f"through the blessed runner "
                        f"(``python -m universal_agent.scripts.vp_apply_and_checkpoint``) "
                        f"so a checkpoint is recorded this time.\n"
                    )
