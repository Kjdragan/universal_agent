...
def _demo_factory_override_block(*, video_title: str, video_url: str) -> str:
    ...
    return (
        ...
        "it). ``--cody-mode hybrid`` runs the build (the /goal loop that writes "
        "the demo's code) on Anthropic-Max for higher-quality code, while the "
        "verify/runtime inference phases run on the ZAI/GLM coding-plan proxy "
        "for cheap demo inference (operator decision 2026-07-02). ``--video`` "
