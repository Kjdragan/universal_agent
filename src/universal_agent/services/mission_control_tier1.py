...
def _llm_prompt(evidence: dict[str, Any]) -> str:
    ...
    return (
        ...
        "Your PRIMARY mission: surface INTELLIGENCE — what the autonomous system\n"
        "is actually DOING and PRODUCING for the operator. The operator does not\n"
        "want a status panel; they want a feed of what landed today.\n\n"
        "Order of priority when picking cards (highest first):\n"
        "  1. INTELLIGENCE WINS — work the system completed, artifacts produced,\n"
        "     reports generated, missions that landed, PRs opened, content ingested,\n"
        "     ideas captured. Use severity=success or informational. These are\n"
        "     the cards the operator most wants to see.\n"
        "  2. NOTEWORTHY DISCOVERIES — patterns the system surfaced, novel\n"
        "     observations from CSI/scrapers, failure patterns worth understanding\n"
        "     (not individual failures — patterns across runs).\n"
        "  3. ACTIONABLE ATTENTION ITEMS — stuck tasks, blocked missions, things\n"
        "     the operator must decide. Severity=warning when serious enough to\n"
        "     interrupt, watching when just tracking.\n"
        "  4. INFRASTRUCTURE PROBLEMS — only if a tier-0 tile alarm needs human\n"
        "     narrative beyond the mechanical card tier-0 already creates.\n\n"
        "DO NOT just summarize raw logs. Empty list is fine if the system is\n"
        "genuinely calm — better to say nothing than invent.\n\n"
