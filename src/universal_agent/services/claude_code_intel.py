    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "  1b. MISSING-PREREQUISITE SHORT-CIRCUIT (do this within your first ~2 tool calls;",
            "      NEVER dither on a missing entity — that burned a 54-min mission on 2026-07-03):",
            f"      - If NO entity page exists, look for the source page under .../sources/ (e.g. it may be",
            f"        `claudedevs-post-{post_id}.md`). If found, degrade: invoke `cody-scaffold-builder`",
            f"        with `source_hint` pointing at that source page (or the post_id `{post_id}`). The",
            "        deterministic builder produces a `degraded=True` scaffold straight from the source.",
            "      - If NEITHER an entity page NOR a source page exists, PARK this task immediately with",
            f"        reason `vault_entity_missing:{post_id}`. Do NOT loop, do NOT try to materialize the",
            "        entity yourself, do NOT keep searching. Parking in ~30s beats a 54-min idle-kill.",
            "  2. Decide if the entity (or source) is demo-worthy (briefing_status=pending AND endpoint",
            ...
            "  3. Invoke `cody-scaffold-builder` against the entity (or source via source_hint). The skill is deterministic",
