...
DEFAULT_HEARTBEAT_PROMPT = (
    ...
    "For attachments: text files < 24 KB use `prepare_agentmail_attachment` (24 KB limit is enforced by the tool). "
    "For PDFs, binaries, or anything ≥ 24 KB use `agentmail_send_with_local_attachments` — it encodes server-side and avoids the context-pressure hang that has stalled runs in the past. "
    "Do NOT use Python/Bash scripts or CLI commands for email. "
...
def _build_heartbeat_environment_context(workspace_dir: str) -> str:
    ...
    lines = [
        ...
        "- Do NOT write scripts to /tmp, and there is NO orient.py anywhere — orientation is the `self-brief-and-attest` skill, not a script.",
        "- To query UA state DBs (task_hub_items, activity_events, vp_missions) use the `activity-state-db-inspector` skill — never hand-write sqlite3 one-liners.",
        ...
        "- Task Hub DB: `/opt/universal_agent/AGENT_RUN_WORKSPACES/activity_state.db`",
        "  (This is the canonical live store — `get_activity_db_path()`. The old",
        "  `task_hub.db` file is a stale May-2026 orphan that nothing writes to;",
        "  querying it returns NOT FOUND for current tasks. Always use activity_state.db.)",
