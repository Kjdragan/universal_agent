...
class AgentSetup:
    ...
    def _build_system_prompt(self) -> str:
        ...
        return (
            ...
            "   - 📅 **DATE PROPAGATION (CRITICAL)**: ALWAYS include the current date in your delegation prompt!\n"
            f"     Example: 'Today is {today_str}. Research [topic] focusing on {user_now.strftime('%B %Y')} content.'\n"
            "   - PROCEDURE:\n"
            "     1. **STEP 1:** Delegate to `research-specialist` using `Task` IMMEDIATELY.\n"
            f"        PROMPT: 'Today is {today_str}. Research [topic]: execute searches for {user_now.strftime('%B %Y')}, crawl sources, finalize corpus.'\n"
