...
class UniversalAgent:
    ...
    def _build_subagent_prompt(self, workspace_path: str) -> str:
        ...
        prompt = (
            ...
            "- It also creates `search_results_filtered_best/` with cleaned crawl files.\n"
            ...
            "### Step 2C: Research Notes Extraction (MANDATORY - DO NOT SKIP)\n"
            "⚠️ **BEFORE writing the final report, you MUST write research notes first.**\n\n"
            "**Why?** This prevents output generation failures and ensures quality.\n\n"
            f"**Write to:** `{workspace_path}/work_products/research_notes.md`\n\n"
            "**Your notes MUST include (be thorough, not just a summary):**\n"
            "1. **Key Quotes** - Extract 10-20 direct quotes with source attribution\n"
            "2. **Statistics & Numbers** - ALL specific figures, percentages, dates, counts\n"
            "3. **Timeline of Events** - Chronological list of key events with dates\n"
            "4. **Major Themes** - 3-5 themes you've identified across sources\n"
            "5. **Conflicting Perspectives** - Note where sources disagree\n"
            "6. **Source URLs** - List each source URL for citation in report\n\n"
            "**ANTI-DRY-SUMMARY RULE:** Do NOT write abstract summaries. Include the actual quotes, the actual numbers, the actual names. Your notes should feel like annotated source material, not a book report.\n\n"
            "**After writing notes:** Proceed to Step 3 where you will read ONLY the notes file to generate the HTML report.\n\n"
            "### Step 3: Report Creation (FROM YOUR NOTES)\n"
            "Now generate the full HTML report. Your notes are already in context - you just wrote them.\n"
            "Use the quotes, statistics, and themes from your notes to build a rich, well-sourced report.\n\n"
