...
class UniversalAgent:
    ...
    def _build_system_prompt(
        ...
        prompt += (
            ...
            "## 📧 EMAIL & COMMUNICATION\n"
            "- **Simone's own email**: Use the official AgentMail MCP tools directly (`mcp__agentmail__send_message`, `mcp__agentmail__reply_to_message`).\n"
            "  - **Attachments**: small text files (< 24 KB) → call `prepare_agentmail_attachment(path=...)` and pass the returned object in `attachments`. The 24 KB limit is enforced by the tool.\n"
            "  - **PDFs, binaries, or any file ≥ 24 KB** → use `agentmail_send_with_local_attachments(to=..., subject=..., body=..., attachment_paths=[...])` instead. It encodes server-side and bypasses the context window — required to avoid the silent inference hang that occurs when large base64 attachments inflate the context past a fragility threshold.\n"
            "  - Do NOT use bash, curl, SDK scripts, or CLI commands in-session.\n"
    ...
    def _build_research_specialist_prompt(self, workspace_path: str) -> str:
        ...
        prompt = (
            ...
            "**Time Window Rules (MANDATORY):**\n"
            "   - If user asks for 'past N days', do NOT invent hardcoded calendar dates.\n"
            "   - Use `COMPOSIO_SEARCH_NEWS` with `num_days: N` for recency and include exact current-date context in prose.\n"
            "   - If you mention dates in text, they must match the most recent N-day window ending today.\n"
            "   - Avoid stale month/day anchors unless the user explicitly asks for a fixed historical range.\n"
            "\n"
            ...
            "  - ❌ Do NOT call `COMPOSIO_CRAWL_*` tools (Composio crawl path is disabled)\n"
            "  - ❌ Do NOT read or inspect the JSON files yourself\n"
            "  - ❌ Do NOT write any Python scripts to process search results\n"
            "  Just call `run_research_phase` - it handles EVERYTHING.\n\n"
            "### After run_research_phase completes:\n"
            ...
            "- `run_research_phase`: Process ALL search results → crawl → filter → refined_corpus.md\n"
        )
        tool_knowledge = get_tool_knowledge_block()
