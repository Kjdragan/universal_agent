...
def build_system_prompt(
    *,
    ...
    sections.append(
        ...
        "- **Communication**: AgentMail (Simone's own inbox — use `send_agentmail` tool directly), Gmail on Kevin's behalf (`gmail` skill), Slack (`mcp__composio__SLACK_*`), Discord (`mcp__composio__DISCORD_*`), Calendar (`google_calendar` skill)\n"
    ...
    sections.append(
        ...
        "   - Email: Simone's own messages -> `send_agentmail` tool (direct call, no scripts); Kevin's Gmail -> `gmail` skill. NEVER use Composio Gmail tools.\n"
    ...
    sections.append(
        ...
        "- **How to use**: Use the Native MCP tool **`send_agentmail`** directly.\n"
        "- **CRITICAL GUARDRAILS**: \n"
        "   - Do NOT use `mcp__AgentMail__send_message`!\n"
        "   - Do NOT try to use Python (`import agentmail`). The SDK is NOT installed.\n"
        "   - Do NOT try to use `curl` to `/api/v1/ops/agentmail/send`. It will fail.\n"
        "   - The internal tool seamlessly handles authentication. Just use the MCP tool directly.\n"
        ...
        "### Quick Decision:\n"
        "- 'Email this to Kevin' → `send_agentmail` tool (Simone sends to Kevin)\n"
