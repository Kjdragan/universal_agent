...
def build_system_prompt(
    *,
    ...
    sections.append(
        ...
        "- `*` - Local tools (File I/O, image gen, PDF, Task Hub, etc.) -> Call directly\n"
        ...
        "- All tasks, reminders, brainstorm capture, and backlog items go through the Task Hub (`task_hub_task_action`).\n"
    ...
    sections.append(
        ...
        "   - Task capture/brainstorm/reminders -> use Task Hub tool (`task_hub_task_action`).\n"
        "   - Email: Simone's own messages -> official AgentMail MCP tools (`mcp__agentmail__send_message`, `reply_to_message`); Kevin's Gmail -> `gmail` skill. NEVER use Composio Gmail tools.\n"
    ...
    sections.append(
        ...
        "  Use `x_trends_posts` (preferred) or `grok-x-trends` (fallback).\n"
    ...
    sections.append(
        ...
        "- **Run workspace is your home:** Treat `CURRENT_RUN_WORKSPACE/work_products/` as your default target for all files, caches, and deliverables.\n"
        "- **Durable deliverables:** When a final work product is complete and you want to save it permanently, you MUST use the `publish_artifact` tool.\n"
        "- **HOW TO PUBLISH:**\n"
        "  1. Save your completed file or directory to your workspace (e.g., `CURRENT_RUN_WORKSPACE/work_products/report.md`).\n"
        "  2. Call `publish_artifact(source_path=\"...\", skill_or_topic=\"...\", description=\"...\")`.\n"
        "  3. The backend will automatically generate the timestamped path in the persistent `UA_ARTIFACTS_DIR` and copy it securely.\n"
        "- DO NOT try to construct absolute paths using `UA_ARTIFACTS_DIR` directly in file writes. Use the workspace + publish tool workflow.\n"
        "- ALWAYS write a small `manifest.json` in your workspace output directories if it's a complex multi-file task.\n"
        "- **MANDATORY**: For data analysis/charts, ALWAYS save the raw source data (CSV/JSON) alongside visual outputs for auditability.\n"
    ...
    sections.append(
        ...
        "**Small text files (file size < 24 KB):**\n"
        ...
        "  - The 24 KB threshold is **enforced by the tool** — files larger than this will be refused.\n\n"
        "**Anything else — PDFs, PNGs, or any file ≥ 24 KB (and ALWAYS for binary file types):**\n"
        "  - Use `agentmail_send_with_local_attachments` directly:\n"
        "    `agentmail_send_with_local_attachments(to=\"...\", subject=\"...\", body=\"...\", attachment_paths=[\"/path/to/file.pdf\"])`\n"
        "  - This tool encodes files server-side and posts directly to AgentMail, bypassing the context window entirely.\n"
        "  - For replies with attachments: use `agentmail_reply_with_local_attachments` instead.\n"
        "  - Why: base64-encoding inflates a 30 KB PDF to ~41 KB of tool-result content. Multiple "
        "    inline attachments compound this and have empirically caused the next inference call "
        "    to hang silently for 30+ min, leaving the run stuck.\n\n"
    ...
    sections.append(
        ...
        "### Task Hub Tools\n"
        "Use `task_hub_task_action` for task operations:\n"
...
def build_vp_system_prompt(
    *,
    ...
    sections.append(
        ...
        "- `*` - Local tools (File I/O, image gen, PDF, Task Hub, etc.)\n"
