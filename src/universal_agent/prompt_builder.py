...
def build_system_prompt(
    *,
    ...
    sections.append(
        ...
        "- **Communication**: AgentMail (Simone's own inbox — use `mcp__internal__send_agentmail` tool directly), Gmail on Kevin's behalf (`gmail` skill), Slack (`mcp__composio__SLACK_*`), Discord (`mcp__composio__DISCORD_*`), Calendar (`google_calendar` skill)\n"
    ...
    sections.append(
        ...
        "   - Email: Simone's own messages -> `mcp__internal__send_agentmail` tool (direct call, no scripts); Kevin's Gmail -> `gmail` skill. NEVER use Composio Gmail tools.\n"
    ...
    sections.append(
        ...
        "- **Durable deliverables:** When a final work product is complete and you want to save it permanently, you MUST use the `mcp__internal__publish_artifact` tool.\n"
        "- **HOW TO PUBLISH:**\n"
        "  1. Save your completed file or directory to your workspace (e.g., `CURRENT_RUN_WORKSPACE/work_products/report.md`).\n"
        "  2. Call `mcp__internal__publish_artifact(source_path=\"...\", skill_or_topic=\"...\", description=\"...\")`.\n"
    ...
    sections.append(
        ...
        "### Quick Decision:\n"
        "- 'Email this to Kevin' → `mcp__internal__send_agentmail` tool (Simone sends to Kevin)\n"
