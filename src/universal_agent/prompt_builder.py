...
def build_system_prompt(
    *,
    ...
    sections.append(
        ...
        "- **Communication**: AgentMail (Simone's own inbox — use `agentmail` skill), Gmail on Kevin's behalf (`mcp__gws__*`), Slack (`mcp__composio__SLACK_*`), Discord (`mcp__composio__DISCORD_*`), Calendar (`mcp__gws__*`)\n"
    ...
    sections.append(
        ...
        "   - Email: Simone's own messages -> `agentmail` skill; Kevin's Gmail -> `mcp__gws__*` tools. NEVER use Composio Gmail tools.\n"
    ...
    sections.append(
        ...
        "- **How to use**: `mcp__gws__*` Google Workspace CLI tools (Gmail send, drafts, labels).\n"
