    ...
    if crawl4ai_api_key:
        ...
        try:
            ...
            async def crawl_single_url(session, url):
                            ...
                            async with session.post(cloud_endpoint, json=payload, timeout=60) as resp:
                                if resp.status == 429:  # Rate limited
                                    ...
                                    if attempt < MAX_RETRIES - 1:
                                        ...
                                        continue
                                    return {"url": url, "success": False, "error": last_error}
                                ...
                                response_payload = data.get("data") if isinstance(data.get("data"), dict) else data
                                raw_content = (
                ...
                return {"url": url, "success": False, "error": last_error or "Unknown error"}
            
            # Execute concurrent crawl with rate limiting
            sys.stderr.write(f"[crawl_core] Starting crawl of {len(urls)} URLs (max {CONCURRENCY_LIMIT} parallel, {MAX_RETRIES} retries)\\n")
