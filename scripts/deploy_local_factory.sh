...
#   - Infisical machine identity for the target stage environment
#
# What this does:
...
usage() {
    ...
    echo "  --infisical-environment      Infisical stage slug (development|staging|production)"
...
log "Stage env:    $INFISICAL_ENVIRONMENT"
log "Branch:       $BRANCH"
...
# Shared stage secrets are loaded from Infisical at startup.
# Runtime identity is machine-local and written explicitly below.
INFISICAL_CLIENT_ID=${INFISICAL_CLIENT_ID}
INFISICAL_CLIENT_SECRET=${INFISICAL_CLIENT_SECRET}
INFISICAL_PROJECT_ID=${INFISICAL_PROJECT_ID}
INFISICAL_ENVIRONMENT=${INFISICAL_ENVIRONMENT}
UA_RUNTIME_STAGE=${INFISICAL_ENVIRONMENT}
FACTORY_ROLE=LOCAL_WORKER
UA_DEPLOYMENT_PROFILE=local_workstation
UA_MACHINE_SLUG=${UA_MACHINE_SLUG:-kevins-desktop}
EOF
...
os.environ["UA_RUNTIME_STAGE"] = "'"$INFISICAL_ENVIRONMENT"'"
os.environ["FACTORY_ROLE"] = "LOCAL_WORKER"
os.environ["UA_DEPLOYMENT_PROFILE"] = "local_workstation"
os.environ["UA_MACHINE_SLUG"] = os.environ.get("UA_MACHINE_SLUG") or "kevins-desktop"
try:
    from universal_agent.infisical_loader import initialize_runtime_secrets
    result = initialize_runtime_secrets(profile="local_workstation", force_reload=True)
    print(
        "OK: Loaded "
        f"{result.loaded_count} secrets from Infisical "
        f"({result.environment or os.environ.get('INFISICAL_ENVIRONMENT', '?')})"
    )
