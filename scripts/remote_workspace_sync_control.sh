...
DEFAULT_MANIFEST_FILE="${UA_REMOTE_SYNC_MANIFEST_FILE:-${REPO_ROOT}/AGENT_RUN_WORKSPACES/remote_vps_sync_state/synced_workspaces.txt}"
...
if [[ "${COMMAND}" != "off" && "${COMMAND}" != "status" && "${SSH_AUTH_MODE}" == "keys" && -n "${SSH_KEY}" && ! -f "${SSH_KEY}" ]]; then
