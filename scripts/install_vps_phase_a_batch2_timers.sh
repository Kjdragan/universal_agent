...
# scripts/install_vps_phase_a_batch1_timers.sh, but batch 2 has a 4-service /
# 6-timer shape: the three proactive-report slots share ONE
# universal-agent-proactive-report.service (driven by three timers), and the
# digest / intel-promoter / codie-cleanup jobs each have their own pair.
#
...
# 4 oneshot services (proactive-report is shared by the three report timers).
SERVICES=(
...
# 6 timers (three report slots + digest + promoter + codie).
TIMERS=(
