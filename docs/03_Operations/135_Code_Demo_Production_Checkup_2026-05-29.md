...
| 3 | **CSI demo-triage → `cody_demo_task` → `/opt/ua_demos`** | **(a) Wired & consumed; live-run auth LIVE-VERIFIED working (2026-05-29)** | operator-approved triage drawer → `cody_scaffold_request`; Simone scaffolds → `cody_dispatch.dispatch_cody_demo_task` (`:45-150`) | **Cody** builds in `/opt/ua_demos/<id>/`; Simone evaluates (`cody_evaluation.py`) per `HEARTBEAT.md:216-242` | "claim ≤1 per cycle"; endpoint discipline `manifest.endpoint_hit==endpoint_required` | create: **SILENT** · complete: vault `## Demos` bullet + dashboard drawer + batched digest · fail: **SILENT** (real-time) |
...
### Path 3 — CSI demo workspace → `/opt/ua_demos` — ROOT CAUSE CORRECTED (twice), then LIVE-VERIFIED WORKING
...
**LIVE VERIFICATION (2026-05-29, later same day) — Path 3 is healthy on the current gateway.** Re-ran the real `claude-code-hooks` demo's `run_demo.sh` under exact production conditions (Infisical prod secrets injected → `CLAUDE_CODE_OAUTH_TOKEN` present, `ANTHROPIC_*` scrubbed exactly as `_scrubbed_env` does, on a throwaway copy so the real workspace was untouched). Result: `claude -p` exit 0, `bash_commands.log` non-empty (PreToolUse hook fired in a live session), manifest `endpoint_hit=api.anthropic.com, status=success, live_status=live_ok`, `LIVE_HOOKS_OK` — **no 401, no skip.** So the child `claude` authenticates against the native Anthropic endpoint when the valid env token is present, which it now is (token created 2026-05-17, v2 2026-05-26; `initialize_runtime_secrets` injects it into the gateway env). The historical 401s (computer-use 05-27, claude-code-hooks 11:14 AM 05-29) were credential-availability gaps that predate the current token+loader state. **Net: no Path 3 code fix is warranted — the "blocked" framing above is historical; live-run now passes.** (Caveat: the verification injected the token via `infisical run`; the code paths `initialize_runtime_secrets`/`_build_cli_env`/`_scrubbed_env` were confirmed to forward it, but the live gateway process's in-memory env wasn't directly read — `/proc/environ` is invalid for runtime-injected secrets. The next organic `cody_demo_task` dispatch closes that last 1%.)
