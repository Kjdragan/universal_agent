...
Fourteen invariants ship today. Each fails open (returns no finding when the data store isn't deployed yet) and most have a time-of-day gate so probes don't false-fire before the underlying cron has had a chance to run.
