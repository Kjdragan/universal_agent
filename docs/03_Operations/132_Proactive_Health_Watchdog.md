...
**Last updated:** 2026-05-26 (zai_inference_health now reads P7 events JSONL — closes the detector blind spot) — The `zai_inference_health` invariant previously read only the rate-limiter snapshot (`AGENT_RUN_WORKSPACES/zai_inference_state.json`), which is populated solely by `with_rate_limit_retry`-wrapped callers. Direct-httpx callers — `services/session_dossier.py`, `discord_intelligence/relevance_filter.py`, and any future caller that bypasses the wrapper — never updated the snapshot, so the invariant was blind to their bursts (the 2026-05-21 session_dossier 49-burst produced 0 watchdog findings even though the semaphore root-cause-fixed it the next day). The invariant now ALSO tails `AGENT_RUN_WORKSPACES/zai_inference_events.jsonl` over rolling windows (10 min for 429s, 30 min for FUP), counts `category in (rate_limited_429, fup_signal)`, and surfaces top callers in `observed_value.events_429_top_callers` / `events_fup_top_callers` so the alert headline names the responsible file. New env knobs: `UA_ZAI_EVENTS_429_WINDOW_SECONDS` (default 600), `UA_ZAI_EVENTS_429_CRITICAL_COUNT` (default 3), `UA_ZAI_EVENTS_FUP_WINDOW_SECONDS` (default 1800). The snapshot-side conditions remain unchanged for callers that DO go through `with_rate_limit_retry`. Regression tests: `tests/unit/test_zai_inference_health.py::test_events_*`.

**Previously:** 2026-05-25 (PR #449 — new `proactive_brief_task_funnel` invariant catches the silent-suppression failure class where artifacts accumulate but no `task_hub_items` rows are queued; fires when any of `convergence_detection`/`insight_detection`/`tutorial_build` has ≥5 artifacts in 48h with 0 matching tasks. Built in response to the 5-week-undetected preference-gate poison incident — implicit-outcome park signals had silently driven the gate to suppress 100% of convergence/insight briefs since 2026-04-18. See [Doc Proactive Intelligence Pipeline § Phase 6 correction](../02_Subsystems/Proactive_Intelligence_Work_Product_Pipeline.md#implementation-correction-2026-05-24-pr-449) for the full root-cause narrative and the detox migration.)

**Previously:** 2026-05-22 (session_dossier semaphore — root-cause fix for 100% of P7-observed 429s) — A single-slot `asyncio.Semaphore` at module scope in `services/session_dossier.py` now serialises the two racing callers (`hooks_service.py:5520` at :40, gateway reaper `_generate_dossier_on_close` at :10/:40) at the LLM-call boundary. P7 observability (PR #412) captured the evidence: 49 of 49 429s in a 22h window originated from this single file's 3–6-deep parallel bursts; every other prolific caller (`wiki/llm.py`, `discord_intelligence`, `youtube_daily_digest`) already serialises naturally and saw 0 429s in the same window. The fix is one `async with _DOSSIER_SEMAPHORE:` wrap around `_async_call_llm`; the existing `if context_brief.md exists` short-circuits at both call sites already prevent double-work on the same workspace, so the semaphore only adds queueing — never extra LLM calls. Regression test: `tests/unit/test_session_dossier.py::TestGenerateSessionDossierSerialization`.

**Previously:** 2026-05-20 (P0–P4 restoration) — Holistic audit and re-platforming across 7 PRs (#395–#400, #402). Layer 1 cron list populated, Layer 2 invariants re-pointed at `activity_state.db` (was reading the empty `runtime_state.db` due to PR #376/#392 plumbing bugs), 3 new universal sweep invariants (`csi_source_liveness` covering 8 CSI sources, `cron_staleness` covering all 22 production crons, `zai_inference_health` covering 429 pressure / FUP detection / UA process count), critical findings auto-park `proactive_health:*` Task Hub rows, persistent warns escalate to Task Hub after 3 ticks, ZAI rate-limiter state persisted to JSON snapshot readable from heartbeat daemon subprocesses. See `lrepos/universal_agent/plans/create-a-plan-to-gentle-pebble.md` for the full restoration plan including §8 (P4 ZAI inference health).

**P7 universal httpx hook (PR #412/#414):** A module-bootstrap monkey-patch in `services/zai_observability.py` wraps `httpx.Client.__init__` and `httpx.AsyncClient.__init__` so every Anthropic-SDK-mediated call from any caller in the process emits a JSONL event at `AGENT_RUN_WORKSPACES/zai_inference_events.jsonl` (10k-line rolling cap). Each event records caller file/function (after walking past `/.venv/`, `/site-packages/`, `/anthropic/`, `/openai/`, `/google/genai/` frames so attribution lands on the real UA caller), URL, status, latency, and category (`ok`, `rate_limited_429`, `fup_signal`, `client_error_4xx`, `server_error_5xx`). The `zai_inference_health` invariant remains the actionable signal — P7 is the forensic record that lets operators see who-called-what when an invariant fires. The session_dossier fix above is the first concrete root-cause win from P7 data.
...
Fifteen invariants ship today. Each fails open (returns no finding when the data store isn't deployed yet) and most have a time-of-day gate so probes don't false-fire before the underlying cron has had a chance to run.
...
| `zai_inference_health` | critical (or warn for process-count-only) | **(P4 #402)** ZAI inference health probe. Reads the `ZAIRateLimiter` snapshot persisted at `AGENT_RUN_WORKSPACES/zai_inference_state.json` after every `record_*` call AND counts UA Python processes via `pgrep`. Fires CRITICAL on (a) 3+ consecutive 429s (sustained throttle), (b) ANY FUP / fair-use / concurrency-violation signal in last 30 min (ban risk), (c) `backoff_floor` saturated at max. Fires WARN-only when just the UA process count is above soft limit (default 30). One finding lists every triggered condition in `observed_value.triggered_conditions`. Cost: 1 KB JSON read + 1 `pgrep` call per heartbeat — no AI inference, no DB writes. FUP detection: `record_fup_signal()` is a new path on the rate limiter, called when the retry wrapper detects FUP keywords (`fair use`, `fup`, `policy violation`, `abuse`, `concurrency limit`, `1313`, etc.) — those errors are NOT retried since retrying worsens ban risk. |
| `csi_source_liveness` | critical | **(P1a #398)** Universal CSI adapter freshness invariant — one probe covering all 8 monitored sources (`hackernews`, `csi_analytics`, `youtube_channel_rss`, `youtube_playlist`, `reddit_discovery`, `threads_owned`, `threads_trends_seeded`, `threads_trends_broad`). Fires when any source's `max(occurred_at)` exceeds its per-source expected-max-silence threshold OR has zero events in the last 30 days. Replaces six potential per-source invariants with one finding that lists every stale source in `observed_value.stale_sources` (operator gets full picture per alert, not 6 emails). **Producer-flag-gated:** a source whose producer is intentionally parked is excluded from evaluation so it can't fire a false CRITICAL — `hackernews` only when `UA_HACKERNEWS_SNAPSHOT_ENABLED` is on (re-parked 2026-06-09; its snapshot path is the only producer), and the Threads lanes only when `UA_CSI_THREADS_LANES_ENABLED` is on. Flip the flag back on and monitoring resumes automatically. |
...
| `proactive_brief_task_funnel` | warn | **(PR #449, 2026-05-25)** For each tracked proactive `source_kind` (`convergence_detection`, `insight_detection`, `tutorial_build`): fires when `proactive_artifacts` produced ≥5 rows in the last 48h but `task_hub_items` has 0 rows with the same `source_kind` in the same window. Catches the silent-suppression class — preference gate poison, queue insert dedup bug, dropped-write race — where artifacts are written but no consumer-claimable task ever materialises. Built in response to the 2026-04-18 implicit-park poison incident that hid for ~5 weeks because every other check (cron freshness, artifact freshness, even `csi_convergence_sync_freshness`) stayed green. One finding lists every starved pipeline in `observed_value.starved_pipelines`. |
