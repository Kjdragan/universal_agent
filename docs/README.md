...
- **[Hacker News — Phase 2 Plan](integrations/hackernews_phase2_plan.md)**: Two leveraged Phase 2 integrations: (Lane A) Pulse → Simone briefing context — fold the weekly HN watchlist signal into the daily autonomous briefing prompt as deterministic markdown (no new LLM call). (Lane B) Movers → CSI lane — emit `hackernews_movers_signal` events from `build_snapshot()` into the existing csi.db events table with materiality filter (`status=new`, `delta≥3`, high-score drops, top-3 controversial) + 24h dedup. Awaiting `/grill-me`. ~580 LOC product+test. Both lanes plug into existing infrastructure — no new pipelines, no new LLM surfaces.
