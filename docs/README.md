...
- **[Proactive Autonomous Work Product Project](02_Subsystems/Proactive_Autonomous_Work_Product_Project.md)**: Active implementation reference for the autonomous work-product project — Task Hub proactive history read model, session/evidence recap storage, three-panel audit links, CODIE cleanup cron lane, feedback/continuation design, and verification record.
- **[Task Hub Dashboard](02_Subsystems/Task_Hub_Dashboard.md)**: Frontend design system (`kcd-*` palette, glassmorphism), Kanban component architecture, Mission Control Chief-of-Staff readout (extended by Mission Control Intelligence System), embedded/full Agent Flow visual process display behavior, spotlight and visual preference persistence rules, dispatcher health, forensic task history, canonical execution visibility, and workspace action routing.
- **[Mission Control Intelligence System](02_Subsystems/Mission_Control_Intelligence_System.md)**: Three-tier operator intelligence surface — tier-0 traffic-light tiles (9 tiles, sweeper-polled, threshold-tuned to actual production cadence), tier-1 LLM-discovered narrative cards with feedback (thumbs/snooze/comment) and action buttons (Generate Prompt / Send to Codie), tier-2 Chief-of-Staff readout with Knowledge Ledger feedback, Knowledge Ledger surface at `/dashboard/mission-control/ledger`, smart event titles + hide-by-default filter on `/dashboard/events`. Phases 0–4, 6, 7, 8 shipped 2026-05-04; Phase 5 auto-remediation deferred per 1-week observation gate.
...
- **[ClaudeDevs X Intelligence System](02_Subsystems/ClaudeDevs_X_Intelligence_System.md)**: Canonical subsystem reference for the `@ClaudeDevs` X intelligence lane — official X API polling, packet creation, replay/backfill, linked-source expansion, external Claude Code vault population, candidate ledgers, delivery verification, historical cron cleanup. **v2 (2026-05) extends with:** lifted truncation caps, 28-day brief, full-corpus capability library, wiki Memex primitives (CREATE/EXTEND/REVISE), research grounding subagent, multi-lane config scaffolding, Phase 0 dependency-currency observation, demo workspace scaffolding.
- **[ClaudeDevs X Intel v2 Design](proactive_signals/claudedevs_intel_v2_design.md)**: Full v2 design doc — vault as canonical product, append-dominant Memex maintenance, Phase 0–5 pipeline, parallel-vault backfill, 13-PR implementation sequence, generalization path to Codex/Gemini lanes.
- **[ClaudeDevs X Intel v2 — Remaining Work Plan](proactive_signals/claudedevs_intel_v2_remaining_work.md)**: Living execution catalog — what's shipped (12 PRs), what's left (11 PRs across four phases), cross-reference table back to the original design's PR numbering, deferred-decision resolution map. Updated after every ship.
- **[CSI V3 Backfill Restart Handoff (2026-05-08)](proactive_signals/csi_v3_backfill_restart_handoff_2026-05-08.md)**: **Self-contained handoff context** for a new Claude conversation that picks up the V2 (now V3) Twitter intelligence backfill work. V3 = V2 architecture + a self-imposed rate-limiting / pacing layer (`csi_llm_pacing.py`) added 2026-05-08 after observed Z.AI proxy slowdowns during sustained-burst LLM calls. Documents what's deployed (Phases A–F shipped, all docs landed) vs sitting in a worktree (`csi_llm_pacing.py` + `paced_llm_call` wraps + 15 unit tests, NOT yet merged at `/tmp/ua-wt-csi-pacing/`); the throttling root cause (China-peak overlap = US night); a known-unresolved logging bug where pacing telemetry doesn't appear in actual subprocess output despite working in standalone tests; three options for the new session to pick from (stderr-print fallback, investigate logger plumbing, or skip pacing entirely and rely on schedule shift); and a concrete pacing config + launch-window recommendation. Pasteable as the opening prompt of a new conversation.
...
- **[2026-05-07 Codie Rogue-Branch Recovery Postmortem](operations/2026-05-07_codie_rogue_branch_recovery.md)**: Incident postmortem for the production recovery on 2026-05-07. A docstring-cleanup `vp_mission` was claimed by Simone's heartbeat daemon and executed in-place against `/opt/universal_agent`, crashing CSI cron at 08:00 CDT with a mid-flight `SyntaxError`. Documents the timeline, what broke, four diagnostic dead ends, the recovery sequence (capture-before-reset → gateway stop → park-not-cancel SQL → `git reset --hard origin/main` → service restart → manual CSI fire), and why each gate was needed.
- **[2026-05-07 Open Follow-ups](operations/2026-05-07_open_followups.md)**: Three durable follow-ups surfaced by the 2026-05-07 incident — (1) reconciler ignores `TERMINAL_STATUSES` (P2, workaround: use `parked`), (2) deploy.yml never advances local `main` pointer leaving prod 97 commits behind (P2), (3) Simone executed a code-author `vp_mission` instead of routing to `vp.coder.primary` (P1, recurrence risk).
- **[2026-05-08 Z.AI Peak-Time Scheduling Finding & Remediation](operations/2026-05-08_zai_peak_time_scheduling.md)**: Active architectural finding. The Z.AI proxy is capacity-limited during Greater-China peak hours (UTC+8 daytime), which maps to **US night** under our `America/Chicago` cron timezone — the inverse of "run heavy batch overnight" intuition. Documents the vendor-position-vs-observed-behavior gap, audits all 12 system crons against China-time demand (9 of 12 currently fire during peak), proposes a target schedule (US 12:00–17:00 CDT lunch/afternoon = China deep-night off-peak), recommends an operator-idle-detection follow-up, and frames the `csi_llm_pacing` token-bucket module as a backstop rather than the primary fix. **Read before picking `default_cron` for any new LLM-bound proactive job.**
- **[2026-05-08 Hostinger API Token Leak Remediation](operations/2026-05-08_hostinger_token_remediation.md)**: Operator runbook for the Hostinger API token previously committed at `.mcp.json:33` from 2026-02-19 through 2026-05-08 (~78 days, 119 affected refs). Token was migrated to Infisical (`production / HOSTINGER_API_TOKEN`) and the file sanitized in commit `bcc5fd4c`. Documents the threat model (history rewrite cannot fully scrub forks/caches/dangling commits, so revocation is the load-bearing remediation), the **revocation checklist** (Hostinger UI → curl-verify 401/403), and the optional `git-filter-repo` history-rewrite procedure with all the gotchas (force-push, collaborator re-clone, GitHub 90-day cache, deleted-PR refs). **Recommendation: stop after revocation; history rewrite isn't worth the operational cost for this leak class.**
- **[2026-05-09 /ship Pollution & Phase 1 HN Follow-ups](operations/2026-05-09_ship_pollution_and_phase1_followups.md)**: **Highly-visible follow-up index** after the Hacker News Phase 1 ship cycle. Captures the working-tree-pollution problem (`/opt/universal_agent/` is being used as both a production checkout AND an agent scratch directory; `/ship` from this checkout sweeps 245+ untracked files into a chore commit), three fix options ranked by effort (A: habit change → `/ship` from `~/dev/universal_agent`, B: workflow safety gate, C: gitignore + janitor cron), two pre-existing real runtime bugs filed as issues #176 / #177, two deferred Phase 1 implementation follow-ups, and a verification checklist for confirming the Hacker News cron is producing data. **Read first before re-running `/ship` from this checkout.**
...
- **[Heartbeat Issue Mediation and Auto-Triage (2026-03-12)](03_Operations/95_Heartbeat_Issue_Mediation_And_Auto_Triage_2026-03-12.md)**: Canonical operations note for structured heartbeat findings, Simone auto-investigation, memory-guided autonomous remediation decisions, Task Hub/Cody fix handoff, and operator review escalation boundaries.
...
- **[Debugging Lessons Living Document (2026-03-23)](03_Operations/103_Debugging_Lessons_Living_Document.md)**: Living collection of reusable debugging lessons from complex incidents, including target-host verification, browser-profile state failures, SHA-first release verification, post-bootstrap env-mutation failures, Gemini API key tiering for image generation, and NLM-first KB artifact generation mandates.
...
- **[ClaudeDevs X Intelligence Lane Design Handoff (2026-04-19)](03_Operations/117_ClaudeDevs_X_Intelligence_Lane_Deferred_Design_2026-04-19.md)**: Historical design note for the dedicated Claude Code intelligence lane monitoring `@ClaudeDevs`, including tiering, KB/wiki expectations, source-access findings, mirror fallback, and resume checklist.
- **[X API And Claude Code Intel Source Of Truth (2026-04-19)](03_Operations/118_X_API_And_Claude_Code_Intel_Source_Of_Truth_2026-04-19.md)**: Canonical implementation reference for X API development, `@ClaudeDevs` polling, Infisical keys, packet outputs, local Claude Code intelligence KB index, Task Hub tiering, and cron registration.
- **[Project Scaffolder Skill Guide (2026-04-28)](03_Operations/127_Project_Scaffolder_Skill_Guide_2026-04-28.md)**: Canonical operations guide for the `project-scaffolder` skill, including core host dependency and Infisical CLI preflight, structured scaffold metadata, generated project preflight, real-path infrastructure rendering, main-branch initial commits, frontend systemd service generation, canonical `.agents/skills` links, dependency install, GitHub publishing, and remaining operator steps.
- **[Task Forge: Autonomous Skill Generation Pipeline (2026-04-19)](03_Operations/119_Task_Forge_Autonomous_Skill_Generation_Pipeline_2026-04-19.md)**: Canonical source of truth for Task Forge — the meta-skill that converts human intent into structured, reusable task-skills. Covers design philosophy (why agents should discover their own process), the velocity argument, task atomization (Phase 1), the 7-phase pipeline (intent → scaffold → execute → quality gate → archive/promote), critical component failure protocol, input source coverage, post-execution SKILL.md reconciliation, skill maturity model (v0→v3), dispatch integration, hook hardening, skill-creator relationship, recursive learning loop, and case studies (paper-to-podcast, gemini-tts-narrator).
- **[ClaudeDevs X Intel VPS Runtime Audit (2026-04-20)](03_Operations/120_ClaudeDevs_X_Intel_VPS_Runtime_Audit_2026-04-20.md)**: Read-only production audit of the first 19-post `@ClaudeDevs` cron packet, downstream Simone work products, Task Hub/artifact state, email verification, tier-count discrepancy, and recommendations for the Claude Code intelligence wiki pipeline.
...
- **[Heartbeat-Safe Proactive Pipeline Operations (2026-04-28)](03_Operations/125_Heartbeat_Safe_Proactive_Pipeline_Operations.md)**: Canonical architecture record for the isolation of the proactive curation pipeline from the system heartbeat using `skip_heartbeat` metadata and `_curation_dispatched` overrides, plus branch synchronization operator rules.
- **[Candidate Ledger Reconciliation Operations (2026-04-28)](03_Operations/126_Candidate_Ledger_Reconciliation_Operations.md)**: Canonical documentation for the Phase 3 completion of the Packet Candidate Ledger, detailing the `reconcile_packet_candidate_ledger` workflow and TDD maintenance protocols.
- **[Artifact Saving and Proactive Work Products (2026-04-30)](03_Operations/128_Artifact_Saving_And_Proactive_Work_Products.md)**: Canonical reference for where autonomous agents must save generated code, demos, and work products on the VPS (e.g., `/home/ua/vpsrepos/`) to prevent loss in ephemeral daemon archives.
...
- **[Demo Execution Environments](06_Deployment_And_Environments/09_Demo_Execution_Environments.md)**: Canonical reference for the dual-environment architecture introduced by the v2 ClaudeDevs intel pipeline — UA repo with ZAI-mapped settings (default, cheap GLM inference) and `/opt/ua_demos/` with Anthropic-native vanilla settings + Max plan OAuth (real Claude features for demos). Explains where each lives on the VPS, how Claude Code picks its environment via project-local settings precedence, common failure modes, and the dependency-currency smoke matrix that gates SDK upgrades across both profiles.
- **[Local Development Environment](06_Deployment_And_Environments/12_Local_Dev_Environment.md)**: **Contract draft (2026-05-10)** — canonical runbook for desktop-based local development. Same code as prod, HTTP/UI surface alive, autonomous loops off, separate dev DB, secrets via Infisical `development` env. Replaces the HQ Dev Lane runbook in `05_Local_Runtime_Modes.md` (which remains canonical for the lane DEFINITION). Includes ON/OFF table, architecture diagram, bootstrap steps, daily workflow, single-iteration loop triggers, troubleshooting, and an explicit "Audit pending" punch list of gaps to close before the contract is fully real.
...
- **[Hermes Kanban + Tenacity vs UA Task Hub Comparison (2026-05-10)](reports/hermes-kanban-tenacity-comparison-2026-05-10.md)**: Investigation report comparing NousResearch Hermes Agent v0.13.0 (Kanban + Tenacity-style retry kernel) against UA's existing Task Hub. Identifies what could be adapted to enhance UA's autonomy/retry posture without rewriting the flow engine. Includes the autonomy-gap correction (§13) and Atlas-throttling finding (§14) that informed the phased adaptation plan.
- **[Hermes Adaptation Phased Plan (2026-05-10)](reports/hermes-adaptation-phased-plan-2026-05-10.md)**: Six-phase implementation plan A-F with code anchors and acceptance criteria. **Shipped:** A.1 (per-task `max_retries`), A.2 (`release_stale_assignments` wired into `dispatch_sweep`), B.1 (operator unstick verbs `rehydrate` / `re_evaluate` / `redirect_to` / `request_revision`). **Remaining:** B.2 (dashboard surface), C (Atlas-direct-dispatch), D (`task_hub_runs` history table), E (Cody Anthropic toggle), F (owned-subprocess observability). Includes "Implementation Status" section as session-resumption anchor.
...
- **[Hacker News — Phase 2 Plan](integrations/hackernews_phase2_plan.md)**: Two leveraged Phase 2 integrations, approved 2026-05-09 via `/grill-me`. (Lane A) Pulse → Simone briefing context — hybrid γ″ architecture: helper does deterministic HN-internal CLI fetches (comment threads + post bodies + Algolia per-topic comment search); Atlas selectively fetches article bodies via the ZAI-native `webReader` MCP during its briefing mission. (Lane B) Movers → CSI lane — emit `hackernews_movers_signal` events from `build_snapshot()` into the existing csi.db events bus with materiality filter and 24h dedup. ~860 LOC product+test. Adds `web-reader` and `web-search-prime` ZAI MCPs to `.mcp.json` as part of the deploy; documents the new "Tool Surface by Execution Mode" matrix in `10_Interactive_Coding_Environment.md`.
