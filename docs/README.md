...
- **[Hacker News — Phase 2 Plan](integrations/hackernews_phase2_plan.md)**: Two leveraged Phase 2 integrations: (Lane A) Pulse → Simone briefing context — once-daily real content hydration (HN comment threads + post bodies + defuddled article excerpts + Algolia per-topic comment search) folded into the daily autonomous briefing prompt as ~15-30K tokens of evidence. The briefing LLM does the synthesis. Comments-and-post-body fallback when articles paywall. (Lane B) Movers → CSI lane — emit `hackernews_movers_signal` events from `build_snapshot()` into the existing csi.db events bus with materiality filter (`status=new`, `delta≥3`, high-score drops, top-3 controversial) + 24h dedup. Awaiting `/grill-me`. ~1,350 LOC product+test. Both lanes plug into existing infrastructure — no new pipelines, no new LLM surfaces.
