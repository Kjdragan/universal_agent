...
- **[Hermes Adaptation Phased Plan (2026-05-10)](reports/hermes-adaptation-phased-plan-2026-05-10.md)**: Implementation plan for the Hermes-derived adaptations. Six phases A-F with code anchors and acceptance criteria. **Shipped:** A.1 (per-task `max_retries`, [PR #193](https://github.com/Kjdragan/universal_agent/pull/193)), A.2 (`release_stale_assignments` wired into `dispatch_sweep`, [PR #194](https://github.com/Kjdragan/universal_agent/pull/194)), B.1 (operator unstick verbs `rehydrate` / `re_evaluate` / `redirect_to` / `request_revision`, [PR #195](https://github.com/Kjdragan/universal_agent/pull/195)). **Remaining:** B.2 (dashboard surface), C (Atlas-direct-dispatch), D (`task_hub_runs` history table), E (Cody Anthropic toggle), F (owned-subprocess observability). Includes "Implementation Status" section as session-resumption anchor.
