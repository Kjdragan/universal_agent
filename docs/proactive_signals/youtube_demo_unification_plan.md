...
- **2026-05-23 (status update — diagnostic findings, no architectural change):**
  - **Path B (`csi_auto_route` → `tutorial_build` → Cody) has been silent for 7 days, but not due to the architecture in this plan.** Audit found 234 cached `tutorial_build_judge` verdicts on prod, all `buildable=0`, mostly `method='no_summary'`. CSI ingested 1,431 YouTube RSS events over the prior 12 days; only ~358 of them got transcript analysis at all, and that ratio collapsed to ~0% from 2026-05-19 onward.
  - **Root cause: CSI transcript-enrich → gateway auth.** The `csi-rss-semantic-enrich.service` calls `http://127.0.0.1:8002/api/v1/youtube/ingest`. Both candidate auth tokens (`UA_YOUTUBE_INGEST_TOKEN`, `UA_INTERNAL_API_TOKEN`) exist in Infisical production env and authenticate correctly when curl'd directly, but the systemd timer's Python process resolves an empty token and sends unauthenticated requests → HTTP 401 → `transcript_status=failed` → `summary_text=''` → judge skips. The script's `_resolve_setting`/`resolve_token_from_infisical` chain does not appear to be producing a value under the timer's environment. Fix candidates: (A) add `UA_YOUTUBE_INGEST_TOKEN` literally to `/opt/universal_agent/CSI_Ingester/development/deployment/systemd/csi-ingester.env`; (B) wrap the `ExecStart` with `infisical run --env=production --` per CLAUDE.md canonical pattern.
  - **The daily digest is NOT routed through Cody today.** The digest cron POSTs top-N candidates to `/api/v1/hooks/youtube/manual` → `youtube-expert` agent → `youtube-tutorial-creation` skill (artifact files on disk). It does NOT enqueue `tutorial_build` Task Hub items. This means the "two parallel systems" framing earlier in this doc remains accurate — they are still two parallel systems even though one of them (Path B) is currently broken.
  - **Trigger conditions updated.** Add a fourth condition before starting PR 14a–d:
    - **(4) Path B has been observed producing organic LLM-judge verdicts** (`buildable=True` or `False` with reasoning, not `method=no_summary`) for at least 7 consecutive days post-ingestion-fix. Without this, the unification has nothing to unify against.
  - **Today's UA-side retirement (separate PR).** `services/youtube_playlist_watcher.py` (a third trigger that fed the same `/api/v1/hooks/youtube/manual` route the digest uses) was retired in branch `claude/retire-yt-playlist-watcher`. It was a continuously-polling poller of `YT_TUTORIALS_PLAYLIST_ID`; the digest produces equivalent (curated) demo dispatches at lower API quota cost. The hook route, the transform, the agent, and the skill are unchanged. This retirement is **not** the PR 14 work — it's just cleanup of one stale trigger.
