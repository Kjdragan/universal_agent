...
| **E.1 + E.2.a + E.3** — Cody Anthropic/ZAI toggle (CLI path + dashboard) | [#229](https://github.com/Kjdragan/universal_agent/pull/229) | pending | 🟢 in flight 2026-05-11 (PR #229 auto-merge); per-task `cody_mode`, resolver, CLI env-scrub, VPAgentHealthTile surface. **E.2.b SDK adapter routing deferred** to follow-up. |
| **F.1 + F.2 foundation** — `worker_pid` + `max_runtime_seconds` schema, helpers, classifier | [#230](https://github.com/Kjdragan/universal_agent/pull/230) | pending | 🟢 in flight 2026-05-11 (PR #230); foundation only — three-site spawn wiring (cron / VP CLI / demo) deferred to per-site follow-up PRs after operator validates this layer. 23 new unit tests. |
