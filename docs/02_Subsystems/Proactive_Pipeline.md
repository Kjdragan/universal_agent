...
**Multi-phase pipeline guard**: Before attempting server-side auto-completion, the gateway inspects the agent's most recent `TodoWrite` tool call for items still in `pending` or `in_progress` status. If any exist, auto-completion is skipped entirely — the agent has explicitly declared more work to do (e.g., Task Forge cascades, multi-step research pipelines), and the session is allowed to continue. This prevents premature termination of complex multi-phase workflows where the agent tracks its own progress via Claude Code's `TodoWrite` tool. Implemented in `_agent_has_pending_todo_items()` and threaded into `_enforce_todo_execution_lifecycle()` via `tool_calls_by_id`.

**Anti-hallucination exit hatch:** Agents are explicitly allowed and expected to use `review`, `block`, or `park` when they cannot finish. These actions must include the concrete failure reason, missing dependency, or next step in the note so the card itself explains why it is not complete. For email/report workflows, explicit `complete` is accepted only after the AgentMail/outbound-delivery ledger proves that final delivery happened; otherwise Task Hub routes the item to `review` with `completion_unverified=true`.
...
| [`proactive_digest_agent.py`](../../src/universal_agent/scripts/proactive_digest_agent.py) | Daily proactive artifact digest — auto-surfaces unseen CODIE PRs, tutorial builds, and convergence insights via email. Reads from `activity_state.db` (fixed 2026-05-22: was reading wrong DB, causing ~638 briefs to pile up undelivered). Cron: 08:00 CST |
| [`reconcile_vp_mission_prs.py`](../../src/universal_agent/scripts/reconcile_vp_mission_prs.py) | VP mission → PR merge reconciler — scans in-progress VP missions, checks GitHub PR merge status, auto-closes tasks when PRs land. Swallows Cloudflare JS-challenges without paging (cron: every 15 min during active hours) |
...
| `main()` (briefings_agent) | `scripts/briefings_agent.py` | Assembles briefing prompt from telemetry + nightly wiki + HN evidence block + demo-triage queue block + ATLAS insight briefs block + watchdog findings block; dispatches morning briefing VP mission |
...
| 13 | **Morning Briefing (telemetry + wiki + HN + triage + ATLAS briefs + watchdog) | Cron (06:30 CST daily) | Once per morning | `scripts/briefings_agent.py` |
