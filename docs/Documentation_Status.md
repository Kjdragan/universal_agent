...
| hackernews_phase2_plan.md | **Phase 2 implementation plan** covering the two highest-leverage items from the Phase 1 catalog. **Lane A:** Pulse → Simone briefing context — extends `briefings_agent.py` with a real once-daily content hydrator (HN comment threads via CLI `items thread`, HN post `text` body, defuddled article excerpts with paywall-graceful fallback, Algolia per-watchlist-topic comment search) that injects ~15-30K tokens of evidence into the VP briefing prompt. The existing briefing LLM does the synthesis per CLAUDE.md's LLM-Native rule. **Lane B:** Movers → CSI lane — wires `build_snapshot()` to emit `hackernews_movers_signal` events into the existing `/var/lib/universal-agent/csi/csi.db` events bus with materiality filter and 24h dedup. ~1,350 LOC product+test, ~700 LOC docs. Six-phase rollout (P2.A1→A3, P2.B1→B3). Awaiting `/grill-me` approval. Revised 2026-05-09 after operator discussion: original "pure markdown formatter" approach (~250 LOC) was rejected because title-level pulse counts feed the LLM aggregates and ask it to imagine evidence — explicitly anti-pattern in CLAUDE.md's LLM-Native rule. |
