...
**Visual contract:** local-only Stitch prototype (`work_products/media/stitch/hackernews/index.html` on VPS) with token map in `README.md`. These artifacts are gitignored and exist only on the VPS filesystem.
...
**Design contract reference for porting:** the Stitch prototype at `work_products/media/stitch/hackernews/index.html` (local-only, gitignored on VPS). Every color token, spacing token, panel layout, and hover affordance maps 1:1. The `README.md` in the same directory is the authoritative token map.
