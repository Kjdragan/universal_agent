...
export function SecurityDashboardTab() {
  ...
  if (report?.error) {
    return (
      <div className="p-6 text-center text-slate-400">
        ...
        <h2 className="text-lg text-slate-200 mb-2">No Security Data Available</h2>
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 text-slate-300 p-6 overflow-y-auto">
      <div className="flex items-center justify-between mb-8 border-b border-slate-800 pb-4">
        ...
        <div className="flex gap-4">
          <div className="bg-slate-900 border border-slate-800 rounded-lg p-3 text-center min-w-[120px]">
            <p className="text-[10px] text-slate-500 uppercase tracking-widest mb-1">Pass Rate</p>
            <p className={`text-2xl font-mono ${passRate >= 90 ? 'text-emerald-500' : passRate >= 70 ? 'text-yellow-500' : 'text-red-500'}`}>
              {passRate}%
            </p>
          </div>
          <div className="bg-slate-900 border border-slate-800 rounded-lg p-3 text-center min-w-[120px]">
            <p className="text-[10px] text-slate-500 uppercase tracking-widest mb-1">Total Tests</p>
            <p className="text-2xl font-mono text-cyan-500">{totalTests}</p>
          </div>
        </div>
      </div>
      ...
      <div className="gap-6 grid grid-cols-1">
        <div className="bg-slate-900/50 border border-slate-800 rounded-xl overflow-hidden">
          <div className="bg-slate-800/50 p-3 border-b border-slate-800">
            <h2 className="text-sm font-bold uppercase tracking-widest text-slate-300">Detailed Findings</h2>
          </div>
          <div className="divide-y divide-slate-800/50">
              ...
              return (
                <div key={idx} className="p-4 flex flex-col gap-2">
                  <div className="flex justify-between items-start">
                     <span className="font-mono text-sm text-cyan-400 max-w-[80%] break-words">
                       {res.prompt?.raw || String(res.prompt)}
                     </span>
                     <span className={`text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-widest shrink-0 ${isVuln ? 'bg-red-500/20 border border-red-500/50 text-red-500' : 'bg-emerald-500/20 border border-emerald-500/50 text-emerald-500'}`}>
                       {isVuln ? 'VULNERABILITY DETECTED' : 'BLOCKED'}
                     </span>
                  </div>
                  
                  <div className="text-xs text-slate-500 bg-black/20 p-2 rounded whitespace-pre-wrap font-mono relative mt-1">
                    <div className="absolute top-0 right-0 text-[9px] bg-slate-800 px-2 py-1 -mt-2 -mr-2 rounded opacity-50 uppercase shadow-md">Agent Output</div>
            ...
            {results.length === 0 && (
               <div className="p-8 text-center text-slate-500">No evaluation results in the payload.</div>
            )}
          </div>
        </div>
      </div>
