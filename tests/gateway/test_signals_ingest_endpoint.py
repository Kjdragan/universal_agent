...
def _headers(payload: dict, *, request_id: str = "req-1") -> dict[str, str]:
...
def _enable_ingest(monkeypatch) -> None:
...
def test_signals_ingest_dispatches_internal_youtube(client, monkeypatch):
    ...
    response = client.post("/api/v1/signals/ingest", json=payload, headers=_headers(payload))
...
def test_signals_ingest_non_youtube_source_skips_internal_dispatch(client, monkeypatch):
    ...
    assert "analytics_internal_dispatches" not in body
    assert hook_stub.calls == []
...
def test_signals_ingest_rss_source_skips_internal_dispatch(client, monkeypatch):
    ...
    response = client.post("/api/v1/signals/ingest", json=payload, headers=_headers(payload, request_id="req-3"))
...
def test_signals_ingest_csi_analytics_is_passive_digest_only(client, monkeypatch):
    _enable_ingest(monkeypatch)
    ...
    payload["events"][0]["subject"] = {
        "title": "Passive CSI trend",
        "window_start_utc": "2026-02-22T00:00:00Z",
        "window_end_utc": "2026-02-22T01:00:00Z",
        "totals": {"items": 2, "by_category": {"ai": 2}},
    }
    response = client.post("/api/v1/signals/ingest", json=payload, headers=_headers(payload, request_id="req-4"))

    assert response.status_code == 200
    body = response.json()
    assert body["accepted"] == 1
    assert body.get("csi_digests_captured") == 1
    assert "analytics_internal_dispatches" not in body
    assert hook_stub.calls == []
    assert hook_stub.action_calls == []
