...
_CHANNEL_CLAIM_NAME = _launcher._CHANNEL_CLAIM_NAME
...
def test_vault_injected_token_is_dropped_when_session_has_no_claim():
    ...
    note = _reconcile_channel_token(env, inherited=None, claim=None)
...
def test_claimed_token_survives_the_bootstrap_overwrite():
    ...
    note = _reconcile_channel_token(env, inherited=CLAIMED_BOT, claim="8862827743")
    assert env[_CHANNEL_TOKEN_NAME] == CLAIMED_BOT
    assert _CHANNEL_CLAIM_NAME not in env, "the marker must not reach claude"
    assert "8862827743" in note and "kept" in note


def test_ambient_token_without_a_marker_is_dropped():
    """The regression that restarted the 409 storm within minutes.

    `zai` / `claudezai` run claude under `infisical run`, which injects UA's
    TELEGRAM_BOT_TOKEN (and TELEGRAM_ALLOWED_USER_IDS) into the environment.
    Inferring a claim from the token's mere presence made those sessions
    pollers on UA's bot. Only the marker counts.
    """
    env = {_CHANNEL_TOKEN_NAME: UA_BOT, "TELEGRAM_ALLOWED_USER_IDS": "7843395933"}
    note = _reconcile_channel_token(env, inherited=UA_BOT, claim=None)
    assert _CHANNEL_TOKEN_NAME not in env
    assert "no claim marker" in note


def test_marker_naming_a_different_bot_is_rejected():
    """A stale or mismatched marker must not launder someone else's token."""
    env = {_CHANNEL_TOKEN_NAME: UA_BOT}
    note = _reconcile_channel_token(env, inherited=UA_BOT, claim="8862827743")
    assert _CHANNEL_TOKEN_NAME not in env
...
def test_reconcile_never_leaks_the_ua_bot_to_a_claimed_session():
    ...
    for inherited in (None, CLAIMED_BOT):
        env = {_CHANNEL_TOKEN_NAME: UA_BOT}
        _reconcile_channel_token(env, inherited=inherited, claim="8862827743")
