...
def _disable_intel_triage(monkeypatch):
    # These tests pin the candidate-write / CSI-sync mechanics (the legacy
    # always-queue path) and predate the pre-Task-Hub triage gate, which is
    # covered separately in test_proactive_convergence_triage.py. Disabling it
    # here keeps them offline (no real triage LLM call) and asserting mechanics.
