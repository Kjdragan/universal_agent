...
The next session's job: adapt the clone to our purpose and **add** the capabilities we
still need (§4). §3 is the map of what the base already gives us for free; §4 splits
"already in the base (keep/adapt)" from "what we build onto it."

## Base setup — DONE (reproducible clone process)

The QLoop-based base is **already stood up and verified runnable** — the next session
starts from a ready repo, no setup needed:

- **Repo:** `github.com/Kjdragan/eval_harness` (**PRIVATE**) · local `~/lrepos/eval_harness`
- Forked from QLoop with **full git history preserved**; default branch **`master`**.
- **`origin` is DETACHED from the original** → it points to `Kjdragan/eval_harness`,
  **not** `kd_evaluator_harness`. The original stays pristine — never push there.
- Verified: `uv sync` clean, `uv run qloop --help` works. See `BASIS.md` in the repo.

Process used (for the record / to reproduce):
```bash
git clone https://github.com/Kjdragan/kd_evaluator_harness ~/lrepos/eval_harness
cd ~/lrepos/eval_harness
git remote remove origin                 # DETACH — never push to the original
# add BASIS.md (fork note + ground rule)
gh repo create eval_harness --private --source . --remote origin --push
uv sync                                  # install deps; then `uv run qloop --help`
```

**Next session starts here:** `cd ~/lrepos/eval_harness`, read `BASIS.md` + QLoop's
`README.md` / `STATUS.md`, then build per §4b. All work on this repo (branch `master`),
never the original.
...
| artifact | location |
|---|---|
| **eval_harness — OUR BASE (start here)** | `~/lrepos/eval_harness` · github.com/Kjdragan/eval_harness (private) · branch `master` · forked from QLoop, origin detached |
