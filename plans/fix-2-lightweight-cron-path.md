...
**Status:** **Shipping** — implemented 2026-05-19. Originally filed as a P3 followup after the 2026-05-12 Composio-outage incident; promoted to P1 on 2026-05-19 after the same heavyweight bootstrap was found to be the root cause of the dashboard's red **"Gateway unreachable (Request to /api/dashboard/gateway/api/v1/version timed out after 4000ms)"** banner. Internal probe on the VPS showed `/api/v1/version` taking 9.6 s on a fresh request (then 1.7 ms once warm), and external probes consistently saw 7–14 s stalls after 30 s idle — symptoms of the gateway asyncio event loop being blocked while the per-minute `simone_chat_auto_complete` cron ran `gateway.create_session()` (which synchronously discovers Composio apps, builds a 54 KB capability snapshot, loads SOUL, and registers a session dossier).

**Owner:** claude (PR ships with lightweight branch + tests + docs)
**Priority:** P1 — direct operator-facing dashboard impact
**Estimate (actual):** ~250 lines (cron_service branch + registrar param + cron registration + tests) + doc update
