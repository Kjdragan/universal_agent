...
<!-- scope:hq -->
- [ ] CI/CD Health Check — auto-fix-forward (Simone owns; added 2026-05-30)
  - **Why this exists:** the operator should never be the one who notices CI failures. `ci-failure-issue.yml` files a GitHub issue for every failed run of `Deploy`, `PR Validate`, `PR Auto-Merge`, `PR Rebase Watchdog`, `Documentation Audit`, `Nightly Documentation Health` (and auto-closes it when that workflow+branch goes green). You are the autonomous owner of that queue — work it every beat so failures get fixed or escalated without operator involvement.
  - **Sweep (each cycle):** `gh issue list --repo Kjdragan/universal_agent --label ci-failure --state open --json number,title,body,labels` (you have authed `gh` on the VPS as `ua`). Cross-check with `gh run list --repo Kjdragan/universal_agent --status failure --limit 15 --json conclusion,name,headBranch,databaseId,displayTitle` to confirm a failure is still live (the filer's `close-issue` job may already be closing it on a green re-run — if so, skip).
  - **Dedup:** skip any issue already labeled `ci-autofix-dispatched` or `needs-operator` (you already actioned it). Process at most **2** fresh failures per cycle, oldest first; note any backlog in your heartbeat response.
  - **Triage each fresh, still-live failure:**
    - **Code / test / lint / doc-citation failure on an open PR** (`PR Validate`, `Documentation Audit`) → dispatch a **Cody fix mission** via your normal Cody delegation path, scoped (`allowed_paths`) to the failing PR's branch + the files the run log implicates. The mission brief MUST instruct Cody to: (1) reproduce and fix so the failing check passes locally, (2) push to the **same PR branch**, and (3) run `gh pr edit <pr> --add-label ci-autofix` on its PR. The `ci-autofix` label makes `pr-auto-merge.yml` auto-merge the fix once CI is green (without it, a `codie/*` fix PR sits unmerged for hours — defeating the loop). Then label the ci-failure issue `ci-autofix-dispatched` and comment the dispatched mission id so you don't re-dispatch next beat.
    - **Infra / human-needed** (Tailscale/SSH auth, an expired secret or `AUTO_MERGE_PAT`, gws/YouTube OAuth `invalid_grant`, a deploy health-gate timeout, a GitGuardian secret block) → do **NOT** dispatch Cody. Escalate to the operator's Telegram channel (`UA_OPERATOR_TELEGRAM_BOT_TOKEN`/`_CHAT_ID`, the same channel `deploy-notify.yml` uses) with a one-line summary + the run URL, and label the issue `needs-operator`.
  - **Anti-loop / retry cap:** if a failure recurs on an area you already dispatched (`ci-autofix-dispatched` present but the check failed again), do **NOT** re-dispatch — escalate to Telegram with `needs-operator` instead. A fix that can't pass CI in one autonomous attempt needs a human.
  - **Report:** one compact line in the heartbeat response — e.g. `CI: clean`, or `CI: 1 Cody fix dispatched (PR #N), 1 escalated to operator`. This is the signal that replaces the operator forwarding failure emails.
