...
via `get_vp_profile("vp.coder.primary").workspace_root`). Two more tiers, added
2026-07-24 and 2026-07-25, reclaim a **different** tree (the repo's registered
git worktrees) that neither of the first two can see — one on the weekly job
(merged trees, removed whole), one on the daily job (caches only). The split is
deliberate and the two must not overlap; see "How the two worktree tiers
compose" below.

**Scope of the worktree tiers (changed 2026-08-02).**
`vp/worktree_utils.py::worktree_prune_roots` now defaults to **both**
`<repo>/.worktrees` **and** `<repo>/.claude/worktrees`
(`_DEFAULT_PRUNE_ROOT_RELPATHS`), still overridable via
`UA_WORKTREE_PRUNE_ROOTS`. The second directory is where the Claude Code harness
puts every agent worktree it spawns, and defaulting to the first alone meant
both cleanup jobs *enumerated* it and discarded it as out-of-scope: a 6.7 GiB
`.venv` (torch + nvidia + triton) sat there behind an orphan commit — branch on
no remote, no PR — while the VPS reported 90.0% disk and the daily reaper logged
`Reaped 0 … 0.00 MiB` three runs running. Widening the default does not widen
what gets deleted on its own; every per-worktree guard below still has to pass.
...
- **Daily regenerable-artifact reap** — `scripts/vp_coder_regenerable_reaper.py::reap_regenerable_artifacts`, registered as the `vp_coder_workspace_regenerable_reap` cron job (`gateway_server.py::_ensure_vp_coder_regenerable_reap_cron_job`, default `25 6 * * *` CT); migrated 2026-06-30 to the deploy-independent `universal-agent-vp-coder-regenerable-reap.timer` (armed by `scripts/install_vps_phase_a_batch1_timers.sh`) because its in-process cron registration no-op'd in prod. Removes ONLY regenerable names (`.venv`, `__pycache__`, `node_modules`, `.pytest_cache`, `.ruff_cache`, `dist`, `build`, `.next`) from each mission dir — these rebuild via `uv sync` / build, so no evidence window is needed. Direct-child mission dirs modified within `UA_VP_CODER_ACTIVE_MISSION_SKIP_HOURS` (default 6h) are skipped so an in-flight `uv sync` is never raced. The live repo `.venv` and any `activity_state.db` are hard-excluded (resolved-path equality + filename guard). Mission source / manifests / logs / `SOURCES/` / `BRIEF.md` / `ACCEPTANCE.md` / `COMPLETION.md` are always preserved. Added 2026-06-25 after the disk-critical incident where `AGENT_RUN_WORKSPACES/vp_coder_primary_external/` held 30G across 221 mission dirs (19.6G of it regenerable `.venv`) because the weekly pruner is structurally blind to sub-7d `.venv` bloat.
  ...
  **Tracked-content guard (added 2026-07-25 — a precondition for root 2, not a live bug on root 1).** `git ls-files` on `origin/main` returns exactly one path under a regenerable name: `node_modules/.vite/vitest/<sha>/results.json`. Every full repo checkout therefore has a *tracked* `node_modules/`, and all 7 trees under `.worktrees` carry it (measured on prod 2026-07-25) — so reaping `node_modules` by name in that root would delete tracked source. Verified NOT to be firing on the existing VP-coder root: all 25 mission dirs sit on refs predating that file (`git ls-files -- node_modules` is empty and no `node_modules/` exists), so the guard is a no-op there today. The fix is one `git ls-files` per mission dir via `vp/worktree_utils.py::tracked_artifact_dirs`, fail-closed (undeterminable ⇒ reap nothing in that dir). It is not cosmetic: a dirty tree permanently fails guard 6 of the merged-worktree prune, so the daily job would otherwise have mechanically disabled the weekly one.
...
- **Weekly merged-worktree prune** — `scripts/vp_coder_workspace_pruner.py::prune_merged_worktrees`, called from that same weekly job's `main` (additive; it never aborts the two tiers above and the job still always exits 0). Removes *registered git worktrees* under `<repo>/.worktrees` whose branch is provably merged. This is a different tree from the two tiers above: re-measured 2026-07-25, `/opt/universal_agent/.worktrees` holds 8,772,672,614 B (8.17 GiB) across **7** worktrees — still the largest single consumer under `/opt/universal_agent`, larger than the live `.venv` (6.9 GiB) — and nothing in the estate had ever reclaimed it, because both existing tiers scope to the VP-coder profile root. **The 2026-07-24 figure in the previous revision of this bullet ("15,700 MiB across 8 worktrees, 13.25 GiB of it two `.venv` trees") went ~7 GiB stale within a day** — an autonomous proactive-health run removed the 8th worktree (`codie-except-logging`, MERGED) at 2026-07-25 00:06 UTC. Re-measure before quoting; the estate moves underneath this doc.
  ...
  **The merged predicate is deliberately a union of two proofs.** `git merge-base --is-ancestor <head> origin/main` (against a freshly-fetched base ref) is checked first, but this repo squash-merges every PR, so a merged branch's tip is never an ancestor of `main` — measured 2026-07-24, `--is-ancestor` reported NOT-merged for all 8 worktree branches *including* two whose PRs (#1482, #1415) were definitively merged. An `--is-ancestor`-only rule reclaims zero bytes forever while looking healthy in the log. When ancestry says no, the GitHub PR state is consulted via `gh pr list --head <branch> --state merged` (`_gh_pr_merged_at`, spawned with a minimal env allow-list, authenticated from `~ua/.config/gh/hosts.yml`; `GITHUB_TOKEN` is absent from prod `.env` and is not relied on). `--state merged` also means closed-but-unmerged branches are left alone.
  ...
  A worktree is removed only when **all ten** guards hold; anything that errors or is unknown is a SKIP (fail-closed), and every candidate is logged with its removal-or-skip reason: (1) it came from `vp/worktree_utils.py::list_registered_worktrees` (`git worktree list --porcelain`) — never a filesystem walk; (2) not the main working tree, not `bare`; (3) its parent is in the allowlist (`UA_WORKTREE_PRUNE_ROOTS`, default both `<repo>/.worktrees` and `<repo>/.claude/worktrees`) — load-bearing, since nine registered worktrees live under `AGENT_RUN_WORKSPACES/vp_coder_primary_external/**` where the other two tiers are already moving/deleting dirs; (4) not `detached`; (5) not `locked`; (6) `git status --porcelain` empty — the gate that protects uncommitted work, and it applies to **both** lanes below; (7) `refs/remotes/origin/<branch>` resolves and nothing is unpushed (a missing tracking ref is ambiguous ⇒ skip, which is why `_fetch_base` fetches one branch and never `--prune`s) — **merged lane only**, see the three-state note; (8) merged by either proof above; (9) quiescent — merge age **and** the worktree *directory's* mtime both older than `UA_WORKTREE_PRUNE_MIN_MERGE_AGE_HOURS` (default 24h); *file* mtimes are unusable here because one repo-root `ruff check .` rewrote `.ruff_cache` inside six worktrees at one identical instant; (10) no running process has its cwd inside the tree (`vp/clients/claude_cli_client.py` launches the CLI with `cwd=<workspace_dir>`). Removal goes through `worktree_utils.py::teardown_worktree(force=False)` so git independently refuses a dirty/locked tree, and `git worktree remove` keeps the branch ref — a mistaken removal loses no commits. An unconditional `git worktree prune` runs first to clear registrations whose directory is already gone.

  **THREE branch states, not two (added 2026-08-02).** `gh_pr_merged_at` answers `None` both when a PR is open and when **no PR was ever opened**, and this tier used to skip both forever. `vp/worktree_utils.py::gh_pr_exists` (`gh pr list --head <branch> --state all`) tells them apart, returning `True` / `False` / `None`-for-unknown, and `prune_merged_worktrees` routes on the answer: **merged** → the ten guards above (unchanged); **a PR exists but is not merged** → skip, unchanged and at any age, because work in flight is not ours to delete; **no PR at all** → the NO-PR lane, which ages the tree out behind four gates, each stricter than its merged-lane counterpart. `None` (couldn't ask) skips. Records carry a `lane` key and the summary counts removals per lane.

  **The NO-PR lane's four gates** (`UA_WORKTREE_PRUNE_NO_PR_MIN_AGE_DAYS`, default **7 days**; kill switch `UA_WORKTREE_PRUNE_NO_PR_ENABLED`, default on): **N1** the worktree directory's own mtime is at least that old; **N2** the newest mtime *anywhere beneath it* — caches, `.git`, `.venv` and `node_modules` excluded, via the reaper's `_newest_activity_mtime` — is at least that old too, because the root's mtime does not move when a session edits a file three levels down and the harness's agent worktrees under `.claude/worktrees` can be genuinely active while committed-and-clean; **N3** the branch **tip commit** is at least that old; **N4** no process has its cwd, exe, **or a mapped file** inside the tree (`live_process_inside(deep=True)` — stricter than the merged lane's cwd-only check). Every no-PR decision is logged at WARNING with the branch, the commit SHA and all three ages. Guard 7 is deliberately *not* applied here and cannot be: a branch that never opened a PR has by definition never been pushed, so demanding a remote-tracking ref would make the lane unreachable. What keeps that safe is that `git worktree remove` deletes the checkout and admin files only — the branch ref and every commit on it survive in the repo.

  **Ships OFF and dry-run; production arms it.** `UA_WORKTREE_PRUNE_ENABLED` defaults `0` and `UA_WORKTREE_PRUNE_DRY_RUN` defaults `1` in code, but `scripts/deploy/remote_deploy.sh` writes `UA_WORKTREE_PRUNE_ENABLED='1'` / `UA_WORKTREE_PRUNE_DRY_RUN='0'` into the prod `.env` — so changes to this tier reach **live deletions** on the next weekly run. This is the first code in the estate that deletes git-managed state on a timer, and the largest single candidate measured was a 7,060 MiB worktree whose PR merged 19h after its files were last written.

- **Daily worktree regenerable reap** — `scripts/vp_coder_regenerable_reaper.py::reap_worktree_regenerable_artifacts`, the **second root** of the daily job's `main` (its own `try/except`, so a git/gh failure can never abort the mission-dir reap that already ran; the unit still always exits 0). Same allowlisted roots as the weekly prune (`UA_WORKTREE_PRUNE_ROOTS`, default both `<repo>/.worktrees` and `<repo>/.claude/worktrees`), different remit: it removes **regenerable artifacts only** — never a whole worktree, never git metadata, never source — from trees whose PR is still **OPEN** (or that never had one), which is exactly the space a merged-only rule can never reach.

  **The reap set is narrower than the mission-dir one, and the narrowing is the safety argument** (`WORKTREE_REGENERABLE_ARTIFACT_NAMES` = `__pycache__`, `.pytest_cache`, `.ruff_cache`). `.venv` is **excluded because a worktree `.venv` is not, in fact, automatically regenerated**: nothing in UA runs `uv sync` in a worktree (`vp/worker_loop.py` only does `git worktree add`; `services/dependency_upgrade.py::run_uv_sync` targets the repo root), so one exists only because a human or agent typed `uv sync` there. Deleting it breaks no service, but it silently bills the next session in that tree a multi-minute, mostly **re-downloaded** rebuild — `~ua/.cache/uv` is 317 MB against a 6.5 GiB venv. `node_modules` is excluded because this repo tracks content inside it; `dist` / `build` / `.next` are excluded because they are 0 bytes outside `.venv` in every production worktree and are plausible *tracked* directory names in a full repo checkout (unlike in a generated mission workspace). `UA_WORKTREE_REGENERABLE_REAP_NAMES` can only **narrow** that set, never widen it.

  **The one `.venv` exception — no PR, ever (added 2026-08-02).** `WORKTREE_NO_PR_ARTIFACT_NAMES` = `{.venv}` and nothing else. The rebuild-cost argument above is what keeps `.venv` out of the base set, and that cost is only real when somebody is coming back — i.e. when a PR is open. It is imaginary for a tree with **no pull request at all**, days idle, nothing running in it: nobody is coming back to pay it. So `.venv` enters scope for **one tree at a time**, after that tree has already passed the idle and no-live-process gates, when `vp/worktree_utils.py::gh_pr_exists` answers a definite `False` (`None` = unknown = `.venv` stays), and only if the `.venv` directory's **own** mtime is past the idle window too — the tree-level quiescence probe deliberately ignores `.venv`, so a venv rebuilt yesterday inside an otherwise-still tree would otherwise qualify. Kill switch: `UA_WORKTREE_REGENERABLE_REAP_NO_PR_VENV_ENABLED` (default on, inside a job that itself ships off). Do **not** add other names to that set: `node_modules` holds tracked content in this repo, and `dist` / `build` / `.next` are plausible tracked directory names in a full checkout.

  Guards, all fail-closed and all logged with a reason (`vp/worktree_utils.py` helpers are shared with the weekly prune so the two can never drift): registered-worktree enumeration only; not main/`bare`/repo-root; parent in the allowlist; not `prunable`/`detached`/`locked`; **not merged** (`gh_pr_merged_at` non-`None` ⇒ the weekly prune owns that tree and reclaims strictly more); quiescent for `UA_WORKTREE_REGENERABLE_REAP_IDLE_HOURS` (default **72h**, not the mission-dir 6h) on **both** the worktree directory's own mtime **and** the newest mtime beneath it *excluding* `.git`/`.venv`/`node_modules` and the cache dirs — raw file mtimes are useless here, six of seven production worktrees reported their newest file at one identical instant from a single repo-root `ruff check .`; no live process with its **cwd, exe, or a mapped file** inside the tree (`live_process_inside(deep=True)` — a venv in use is bound by exe/mmap, not by cwd); nothing git-tracked inside the artifact; and the existing live-repo-`.venv` / `activity_state.db` hard excludes. The walker never *enters* `.git`, `.venv`, or `node_modules`, so a hunt for `__pycache__` cannot reach the ~31 MB of it sitting inside a worktree `.venv` we promised not to touch.

  **Ships OFF and dry-run:** `UA_WORKTREE_REGENERABLE_REAP_ENABLED` defaults `0`, `UA_WORKTREE_REGENERABLE_REAP_DRY_RUN` defaults `1`. Known gap that justifies the long idle window: interactive agent sessions on the box run with `cwd=/opt/universal_agent` even while editing a worktree through absolute paths (`~ua/.claude/projects/` has 1,172 transcripts under `-opt-universal-agent` and **not one** project dir for any `.worktrees/*` path), so a live editing session is invisible to every cwd-based check. The weekly prune substitutes "merged PR + 24h" for that missing signal; an open-PR tree has no equivalent proof.

  **How the two worktree tiers compose (read this before changing either).** Daily owns OPEN-PR trees, weekly owns MERGED ones, and the daily job skips merged trees explicitly so they never fight. **NO-PR trees are claimed by both, at different depths and different clocks:** the daily job takes their caches immediately and their `.venv` once idle, while the weekly job removes the whole tree only after the 7-day age-out. They do not conflict — whichever runs first simply leaves less for the other, and the weekly job's clean-tree guard is unaffected because every name either job reaps is gitignored. Honest reach, measured 2026-07-25: the daily reap's **exclusive** claim is **76,861,308 B = 73.3 MiB** across the five open-PR worktrees — the entire 6.55 GiB of regenerable bytes inside `.worktrees` sits in the two MERGED trees, where the weekly job reclaims *more* (6.94 GiB of whole trees, source included). Zero open-PR worktrees have ever been observed holding a `.venv`. Do not repeat the 13.25 GiB framing: it double-counts the weekly tier's bytes and overstates this tier by ~180×. The highest-value next action here is not more code — it is flipping `UA_WORKTREE_PRUNE_ENABLED=1` (dry-run first) on the already-shipped weekly job.
