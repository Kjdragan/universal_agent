...
2026-07-24 and 2026-07-25, reclaim a **different** tree (`<repo>/.worktrees`)
that neither of the first two can see — one on the weekly job (merged trees,
removed whole), one on the daily job (open-PR trees, caches only). The split is
...
- **Daily worktree regenerable reap** — `scripts/vp_coder_regenerable_reaper.py::reap_worktree_regenerable_artifacts`, the **second root** of the daily job's `main` (its own `try/except`, so a git/gh failure can never abort the mission-dir reap that already ran; the unit still always exits 0). Same allowlisted roots as the weekly prune (`UA_WORKTREE_PRUNE_ROOTS`, default `<repo>/.worktrees`), different remit: it removes **regenerable artifacts only** — never a whole worktree, never git metadata, never source — from trees whose PR is still **OPEN**, which is exactly the space a merged-only rule can never reach.
