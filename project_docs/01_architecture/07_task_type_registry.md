...
| `proactive_general` | canonical | Generic proactive Atlas mission. **As of M3 (2026-06-15)** the dispatch entry is the M2 priority dispatcher's prefer-ATLAS lane (`metadata.preferred_vp='vp.general.primary'`), not the retired direct-dispatch sweep — flag-gated/default-OFF in prod (see §2 / §6). | `services/priority_dispatcher.py::classify_task`, `::dispatch_claimed` |
...
| **atlas_direct_dispatch** | **removed** (M3, 2026-06-15) | Was an independent ~60s sweep dispatching `preferred_vp='vp.general.primary'` tasks without Simone's heartbeat throttle (Hermes Phase C). **Cron RETIRED in M3** — the prefer-ATLAS lane is now owned by the M2 priority dispatcher (`services/priority_dispatcher.py::classify_task` + `::dispatch_claimed`), which remains flag-gated and **default-OFF** in prod (`UA_PRIORITY_DISPATCHER_ENABLED` master + `UA_DISPATCHER_PREFER_ATLAS` Stage-B; M3 prepared the staged operator-gated enable but did **not** flip them). The service module `services/atlas_direct_dispatch.py` is **kept importable** — `heartbeat_service` still reads `::list_recent_atlas_direct_dispatches` for Simone's briefing. (see §6) | `services/priority_dispatcher.py::dispatch_claimed` (replaces `services/atlas_direct_dispatch.py::dispatch_atlas_candidates_once`) |
