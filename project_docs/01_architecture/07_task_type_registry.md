...
| `proactive_general` | canonical | Generic proactive Atlas mission. **As of M3 (2026-06-15)** the dispatch entry is the priority dispatcher's prefer-ATLAS lane (`metadata.preferred_vp='vp.general.primary'`), not the retired direct-dispatch sweep. The priority dispatcher itself is **DEFAULT-ON in prod** (`priority_dispatcher.py::priority_dispatcher_enabled` → True since 2026-06-16, PR #1034/#1038); the prefer-ATLAS sub-lane remains **default-OFF** (`UA_DISPATCHER_PREFER_ATLAS`), so general/research work falls back to Simone-first until that toggle is flipped (see §2 / §6). | `services/priority_dispatcher.py::classify_task`, `::dispatch_claimed` |
...
| **Priority dispatcher (D3)** | canonical | **DEFAULT-ON in prod** (`services/priority_dispatcher.py::priority_dispatcher_enabled` → True unless an explicit `UA_PRIORITY_DISPATCHER_ENABLED=0` kill-switch; on since 2026-06-16, PR #1034/#1038). Deterministic, priority-ordered classify-then-dispatch run inside the shared `_enrich_with_routing` step: VP-direct for tagged/classified VP work with a free slot, Simone-first **fallback** for general, chat, ambiguous, and capacity-deferred work. The `UA_DISPATCHER_PREFER_ATLAS` Stage-B toggle (default **OFF**, `::prefer_atlas_enabled`) controls whether the general/research tail prefers ATLAS or falls back to Simone. | `services/priority_dispatcher.py::classify_task`, `::dispatch_claimed`, `services/dispatch_service.py::_enrich_with_routing` |
| **Simone-first routing** | canonical (now the **fallback** path) | When the priority dispatcher is **off** (kill-switch) or returns a general/chat/ambiguous result, the claimed task is routed to Simone, who decides delegation. With the dispatcher default-ON, `_enrich_with_routing` returns early for VP-direct decisions and **skips** `route_all_to_simone`. | `services/dispatch_service.py::_enrich_with_routing`, `services/agent_router.py::route_all_to_simone` |
| **atlas_direct_dispatch** | **removed** (M3, 2026-06-15) | Was an independent ~60s sweep dispatching `preferred_vp='vp.general.primary'` tasks without Simone's heartbeat throttle (Hermes Phase C). **Cron RETIRED in M3** — the prefer-ATLAS lane is now owned by the priority dispatcher (`services/priority_dispatcher.py::classify_task` + `::dispatch_claimed`), whose master switch is now **default-ON** (`priority_dispatcher.py::priority_dispatcher_enabled`, since 2026-06-16); the `UA_DISPATCHER_PREFER_ATLAS` Stage-B sub-toggle stays **default-OFF**. The service module `services/atlas_direct_dispatch.py` is **kept importable** — `heartbeat_service` still reads `::list_recent_atlas_direct_dispatches` for Simone's briefing. (see §6) | `services/priority_dispatcher.py::dispatch_claimed` (replaces `services/atlas_direct_dispatch.py::dispatch_atlas_candidates_once`) |
...
by any of this code, read the row.** For the platform-wide live/parked/paused/retired status of every
subsystem, see the [Platform Status Registry](../00_PLATFORM_STATUS_REGISTRY.md).
...
| **`atlas_direct_dispatch` cron** (standalone */1 prefer-ATLAS sweep) | priority-dispatcher prefer-ATLAS lane (`services/priority_dispatcher.py::classify_task` + `::dispatch_claimed`); master switch `UA_PRIORITY_DISPATCHER_ENABLED` is now **default-ON** (`priority_dispatcher.py::priority_dispatcher_enabled`, since 2026-06-16), Stage-B `UA_DISPATCHER_PREFER_ATLAS` stays **default-OFF** | Redundant with the Pythonic priority dispatcher, which now owns the `preferred_vp='vp.general.primary'` lane. M3 retired the cron; the master switch was subsequently flipped **default-ON** (2026-06-16, PR #1034/#1038) after the D3 path was proven live, while the prefer-ATLAS Stage-B toggle remains default-OFF (general/research falls back to Simone until flipped). Mechanism: `gateway_server.py::_ensure_atlas_direct_dispatch_cron_job` no longer registers the */1 cron — it now DELETEs the persisted row via `cron_service.py::CronService.delete_job` (a plain `enabled=False` disable did not durably stop the live prod row). The module `services/atlas_direct_dispatch.py` stays importable (`heartbeat_service.py` reads `::list_recent_atlas_direct_dispatches` for Simone's briefing); `scripts/atlas_direct_dispatch.py` is also kept. **Coupling side-effect:** each fire emitted a `request_heartbeat_next` wake (`gateway_server.py::_maybe_wake_heartbeat_after_autonomous_cron`, ~60/hr); retiring it removed that source, leaving `simone_chat_auto_complete` (*/1, ~62/hr) as the last autonomous-cron coupling-wake — which **M4 then default-denied**: `_maybe_wake_heartbeat_after_autonomous_cron` now consults a default-deny allowlist (`cron_service.py::coupling_wake_allowed_jobs`, **empty by default**) + ~300s debounce, so no autonomous cron wakes Simone for dispatch (covered out-of-band by the priority dispatcher + `idle_dispatch_loop`; urgent work still wakes via `request_heartbeat_now`). **Concurrency note:** the dispatcher per-agent cap is per-sweep only (`services/vp_capacity.py::_vp_active_counts` vs `UA_MAX_CONCURRENT_VP_GENERAL`, default 2 — a delegated task holds no active VP assignment across ticks); the real execution-concurrency bound on running ATLAS missions is the VP worker — `vp/worker_loop.py::VpWorkerLoop` sets `max_concurrent_missions` via `feature_flags.py::vp_max_concurrent_missions` (`UA_VP_MAX_CONCURRENT_MISSIONS`, default 1, min 1). | M3, 2026-06-15 |
