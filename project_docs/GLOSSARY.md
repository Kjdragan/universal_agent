...
| **CSI** | Creator/Convergence Signal Intelligence — the pipeline that ingests external creator signals (YouTube RSS, Reddit, X/@ClaudeDevs, HN), runs a vault intelligence pass, and detects convergence (`services/csi_intelligence_pass.py`, `csi_intelligence_persistence.py`). DB resolved via `_csi_default_db_path` (split-brain hazard). |
