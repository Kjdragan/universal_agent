...
- **[ADR: Scheduling Substrate Redesign](06_platform/08_scheduling_substrate_adr.md)** — _(active — Phases B/C/D + A batches 1–2 shipped & verified live; A batches 3–4 operator-gated)_ Deploy-resilient scheduling: two-axis substrate policy + per-job table (31 crons → systemd `OnCalendar`+`Persistent` vs in-process), Mission Control sweeper extraction, deterministic health timer + delivery contract, consolidations, and a deploy-window-aware backfill model. Builds on the S1–S4 local fixes. _(verified 2026-06-06)_
