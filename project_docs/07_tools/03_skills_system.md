...
- **Skill invocation is model-agnostic across the harness — ZAI/GLM models invoke skills too (verified 2026-06-22).** Both Anthropic models *and* ZAI/GLM models call the native `Skill` tool when a **real** `.claude/skills/` skill is surfaced — either in-process via the Agent SDK (`query(..., skills="all", setting_sources=["user","project"])`, which adds `Skill` to `allowed_tools`) or via the CLI's native `.claude/skills/` discovery. In a direct test, `glm-5.2` and `claude-opus-4-8` each invoked the same real skill cleanly (`Skill(skill='provision-local-gpu-ollama')`). **Misassumption to avoid (a real one we made):** a skill-triggering *test* that registers a `.claude/commands/<name>.md` (a slash **command**, not a skill) and watches `claude -p` output for a `Skill` tool_use does **not** exercise the skill system — there is no real skill to invoke, so the signal is meaningless. That harness limps along on Opus but returns 0 on GLM, which *looks like* "GLM can't use skills" but is purely a harness artifact. (This is the trap inside skill-creator's bundled `run_eval.py`.) **To evaluate skill triggering, point a real `.claude/skills/` skill through the Agent SDK with `skills="all"`** — not a command file via `claude -p`.
