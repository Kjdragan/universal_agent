...
> **STATUS (updated 2026-06-09).** Agent shell access must clear **two independent gates**: the
> harness *permissions* allow-rule (`Bash(ssh ua@uaonvps:*)`, added 2026-06-06) **and** the
> auto-mode *classifier*. The Bash allow-rule alone was necessary-not-sufficient — the classifier
> kept denying prod SSH until a scoped `autoMode.allow` entry was added **2026-06-09**. **Today:
> read-only `ssh ua@uaonvps "<cmd>"`** (with `dangerouslyDisableSandbox: true` for egress) works
> from an agent for `journalctl`/`systemctl status`/`sqlite3 -ro`. **Prod *writes* over SSH (e.g.
> `sudo systemctl restart`) are still classifier-blocked** (they trip "Remote Shell Writes" /
> "Interfere With Workloads") and remain operator-only via the `!` prefix. The "agents cannot
> self-grant" *principle* in §3 still holds. Prefer Channel 1 (the read-API) for anything it answers.
...
RUN a shell command (sqlite3 on the 2.5 GB DBs,        → ssh ua@uaonvps  OR  tailscale ssh ua@uaonvps
  journalctl, systemctl, grep a unit file)               (READS: 2 operator gates in place — see §3; WRITES operator-only)
...
permission + auto-mode classifier layers** (§3), not the tailnet.
...
## 3. Channel 3 — Shell via SSH / Tailscale SSH (reads enabled, writes operator-only — see §3.1)
...
**Read-only use is live as of 2026-06-09 (two operator gates cleared — see §3.1); just run the
read directly. Prod writes stay operator-only.**

### 3.1 The grant — two gates (reads enabled 2026-06-09)

Agent SSH to prod must clear **two independent gates**, and the first does not satisfy the second:

1. **Permissions allow-rule** (`Bash(ssh ua@uaonvps:*)` + `Bash(tailscale ssh ua@uaonvps:*)`,
   added 2026-06-06) — gate 1. Invoke with **no leading flags** so the prefix matcher hits
   (see §3.2), plus `dangerouslyDisableSandbox: true` for egress.
2. **Auto-mode classifier** — gate 2, independent of gate 1. By default it denies prod SSH as a
   *"Production Read / Remote Shell Write to a shared host."* A scoped `autoMode.allow` entry
   added **2026-06-09** clears it for **read-only** diagnostics on this one host.

- **Reads work today, no `!` prefix:** `journalctl`, `systemctl status`/`show`, `sqlite3 -ro`, `grep`.
- **Writes do NOT:** `sudo systemctl restart` and other prod writes are still classifier-denied
  even though the allow text mentions restarts — they trip "Remote Shell Writes" / "Modify Shared
  Resources" / "Interfere With Workloads" (restarting a VP-worker unit SIGTERMs running missions).
  Hand those to the operator as a `! ssh ua@uaonvps "<cmd>"` one-liner. This gate is *intentional*.
- **An agent still cannot grant *itself* either gate** — editing `~/.claude/settings.json`
  (permissions *or* `autoMode`) is blocked as *self-modification* / *Auto-Mode Bypass*
  ("regardless of user request"). The operator adds both rules; do not try to add them yourself.

### 3.2 The one-time operator action (two settings — operator runs these, not the agent)

**Gate 1 — permissions allow-rule.** Add to `~/.claude/settings.json` → `permissions.allow`:
...
**Gate 2 — auto-mode classifier exception.** Add a scoped prose entry to `~/.claude/settings.json`
→ `autoMode.allow` (keep `"$defaults"` first; do **not** add the host to `autoMode.environment` —
that redefines the exfil trust boundary and makes the box a sanctioned data *sink*). The entry
authorizes read-only `ssh ua@uaonvps` diagnostics on this one host; verify with `claude auto-mode
config`. Added 2026-06-09 — clears reads, not writes.
...
- **READ/probe freely.** `sqlite3 'file:/path?mode=ro' "..."`, `journalctl`, `systemctl status` — these clear the classifier directly (2026-06-09).
- **Don't attempt prod *writes* over agent SSH** (`sudo systemctl restart`, killing units) — still classifier-blocked by design; hand them to the operator via the `!` prefix.
...
- **Agents cannot self-grant SSH** (the principle, both gates) — but the operator added the `permissions.allow` rule (2026-06-06) **and** the `autoMode.allow` classifier exception (2026-06-09), so **read-only** SSH is live now (§3.1/§3.2); prod writes stay operator-only. Invoke SSH with no leading flags so the rule matches.
