...
3. **Remote auth (since 2026-08-03):** the prod checkout's `origin` is `git@github.com:Kjdragan/universal_agent.git`, authenticated by a repo **deploy key** (`/home/ua/.ssh/id_ed25519`, write-enabled). The previous embedded classic-PAT HTTPS remote was removed (credential-in-URL leak surface); `remote_deploy.sh::REPO_URL` (bootstrap-clone fallback) uses the same SSH form, so a fresh-box bootstrap authenticates with the same key. `AUTO_MERGE_PAT` (fine-grained, Actions-side) is unrelated to this credential.
