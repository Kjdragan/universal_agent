...
13. **Install canonical systemd units** from repo templates (`install_vps_systemd_units.sh --lane production`, `install_vp_worker_services.sh`, CSI's `csi_install_systemd_extras.sh`), plus `install_uv_cache_prune_timer.sh` (idempotent; installs + enables `universal-agent-uv-cache-prune.timer`, the daily deploy-independent prune backstop).
