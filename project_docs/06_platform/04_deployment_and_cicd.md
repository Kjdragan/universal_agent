...
code_paths:
  ...
  - deployment/systemd/universal-agent-vp-coder-workspace-pruning.service
...
3. **Remote auth (since 2026-08-03):** the prod checkout's `origin` is `git@github.com:Kjdragan/universal_agent.git`, authenticated by a repo **deploy key** (`/home/ua/.ssh/id_ed25519`, write-enabled). The previous embedded classic-PAT HTTPS remote was removed (credential-in-URL leak surface); `remote_deploy.sh::REPO_URL` (bootstrap-clone fallback) uses the same SSH form, so a fresh-box bootstrap authenticates with the same key. Because the script can run as root, it exports `GIT_SSH_COMMAND` pinning ua's key + known_hosts for every git operation. `AUTO_MERGE_PAT` (fine-grained, Actions-side) is unrelated to this credential.
...
9. **Prune uv caches** — `bash scripts/prune_uv_caches.sh` (non-fatal) prunes all four trees (`ua ~/.cache/uv`, root `/root/.cache/uv`, `/tmp/uv_cache`, `$PROD_DIR/.uv-cache`) with `uv cache prune --ci --force`. `--force` is mandatory: the caches are in near-constant use by long-running `uv run` services (VP workers, csi uvicorn, discord bot) + cron/hook `uv run` churn, so a lock-respecting prune times out even at `UV_LOCK_TIMEOUT=300s`. `--ci` only removes re-downloadable wheels/sdists and keeps the unpacked/built archives that live environments hardlink from, so running services are unaffected. `prune_uv_caches.sh` is the **single source of truth**, shared with the `universal-agent-uv-cache-prune.timer` daily backstop (installed in step 13). This is the durable fix for the disk-usage climb (added 2026-06-04 — see the Gotcha below).
...
13. **Install canonical systemd units** from repo templates (`install_vps_systemd_units.sh --lane production`, `install_vp_worker_services.sh`, CSI's `csi_install_systemd_extras.sh`), plus four deploy-independent backstop-timer installers — `install_uv_cache_prune_timer.sh` (daily uv-cache prune; enables `universal-agent-uv-cache-prune.timer`), `install_vps_service_watchdog.sh`, `install_vps_oom_alert.sh`, and `install_vps_session_reaper_timer.sh` (daily workspace GC at 03:00 CT; enables `universal-agent-session-reaper.timer` — archives stale `AGENT_RUN_WORKSPACES/{cron_*,session_*}` dirs older than 24h, then purges `AGENT_RUN_WORKSPACES_ARCHIVE` entries older than 30 days; added 2026-06-17 after the workspace dir reached 12G+ with no deployed cleaner). The latter two re-`enable --now` the service-watchdog and OOM-alert timers on **every** deploy: those timers are now `OnCalendar`-anchored, and re-installing them each deploy is what keeps them armed across the frequent deploy `daemon-reload`s (the prior monotonic-only anchors went `NextElapse=infinity` and the timers sat dead for weeks — see `ops-vps-recovery` → "Dead-timer drift"). Each backstop installer is behind a non-fatal `|| echo WARN` guard so a timer-install hiccup cannot fail a deploy. `csi_install_systemd_extras.sh` installs only its `CANONICAL_UNITS` and its orphan sweep disables+removes retired `csi-*` units on every deploy (most recently the Threads units `csi-threads-token-refresh-sync` + `csi-threads-semantic-enrich`, retired 2026-07-30).
...
- **Disk-growth drivers under sustained VP-coder load (re-characterized 2026-06-25).** Two co-equal drivers, both with automated reclamation: (1) **VP-coder mission ``.venv`` bloat** under `AGENT_RUN_WORKSPACES/vp_coder_primary_external/` — observed 30G across 221 mission dirs on 2026-06-25 (19.6G of it regenerable per-mission `.venv`, `__pycache__` 0.8G, `node_modules` 0.4G), driven structurally by every fresh mission running `uv sync`. Reclaimed daily by `scripts/vp_coder_regenerable_reaper.py::reap_regenerable_artifacts` (registered as the `vp_coder_workspace_regenerable_reap` cron job — `gateway_server.py::_ensure_vp_coder_regenerable_reap_cron_job`, default `25 6 * * *` CT, dormancy-exempt fixed-time cron kept inside the 06:00-21:00 CT window; migrated 2026-06-30 to the deploy-independent `universal-agent-vp-coder-regenerable-reap.timer` — armed by `scripts/install_vps_phase_a_batch1_timers.sh` alongside its weekly `vp_coder_workspace_pruning` sibling, because its in-process cron registration no-op'd in prod (zero dispatch rows ever)) which removes ONLY regenerable names (`.venv`, `__pycache__`, `node_modules`, `.pytest_cache`, `.ruff_cache`, `dist`, `build`, `.next`) — every one of those rebuilds via `uv sync` / build, so no evidence window is needed; mission source/manifests/logs are preserved, dirs within `UA_VP_CODER_ACTIVE_MISSION_SKIP_HOURS` (default 6h) are skipped, and the live repo `.venv` and any `activity_state.db` are hard-excluded. Whole-dir archival of fully-completed missions is still owned by the **weekly** `scripts/vp_coder_workspace_pruner.py` (`gateway_server.py::_ensure_vp_coder_workspace_pruning_cron_job`, default `5 17 * * 0` CT). (2) **uv cache** (ua `~/.cache/uv`, root `/root/.cache/uv`, `/tmp/uv_cache`, `$PROD_DIR/.uv-cache`) — was the dominant driver on 2026-06-04 (reached ~65G, drove disk 70%→78% in ~2 weeks), but `scripts/prune_uv_caches.sh` (step 9) now prunes all four every deploy with `uv cache prune --ci --force`, and the same script runs daily via `universal-agent-uv-cache-prune.timer` (step 13) as a deploy-independent backstop. The uv cache is regularly <1G now, so `uv cache prune` often finds nothing unused — the older "uv cache is THE driver" / "AGENT_RUN_WORKSPACES dirs are NOT the driver" framing is stale. **`--force` is non-negotiable** for the uv-cache prune: the caches are locked nearly continuously by long-running `uv run` services + cron/hook churn, so a lock-respecting prune times out even at `UV_LOCK_TIMEOUT=300s` (measured); `--force` skips the in-use lock wait and still only removes re-downloadable wheels/sdists (a 64.7G manual reclaim on 2026-06-04 left gateway/api/webui/vp-workers/csi all healthy, disk 78%→58%). The `disk_usage_health` watchdog invariant (`disk_usage_health.py::disk_usage_health`) **no longer names any driver a priori** — see the next bullet.
- **The disk watchdog measures; it does not guess (rebuilt 2026-07-15).** The invariant used to hardcode a "driver" narrative and scan a four-entry leaf allowlist (`AGENT_RUN_WORKSPACES`, `_ARCHIVE`, `/opt/ua_demos`, `ua_scratch_archive`). Both failed at once during a 91.4%-full critical: it reported a **0.04G** dir as the top consumer while **7.2G** sat in `vp_coder_primary_external` (inside a root it *was* scanning) and ~60G sat in roots it had never heard of — `/home/ua/.cache` (18G), `/opt/universal_agent/.worktrees` (16G), `/home/ua/.claude-science` (12G), `/var/lib/containerd` (9.3G). Two independent defects: (1) the leaf allowlist covered ~17G of a 170G-used disk, so anything nobody thought to add was invisible; (2) the Python byte-walk shared one 2s / 50k-entry budget across all roots, blew it, and then reported the **partial sums as if they were real sizes** — so the "top consumers" list was really "whichever dirs `scandir` reached first." Now: `_consumer_roots` returns the *parent* dirs everything lives under (`/opt/universal_agent`, `/opt`, `/home/ua`, `/var/lib`, `/tmp`, `/root`), so new consumers are **discovered**, not enumerated; `_du_children` shells to `du -x --max-depth=1` (accurate, sees dot-dirs, one filesystem, ~20-30s box-wide); and `_cached_top_consumers` serves a 6h-TTL cache while refreshing on a **background thread**. That last part is load-bearing: `gateway_server.py::ops_proactive_health` is `async def` and awaits the probe **on the event loop**, so the probe must never walk the filesystem (measured: 0.45ms cold / 0.03ms warm; a full scan is ~20s). Knobs: `UA_DISK_HEALTH_ROOTS`, `UA_DISK_HEALTH_SCAN_TTL_S`, `UA_DISK_HEALTH_SCAN_TIMEOUT_S`, `UA_DISK_HEALTH_SCAN_TOP_N`. Additions 2026-07-23 (top-9 tasks 1c+3): `_du_children` now runs `sudo -n du` first (root-owned dirs like `/var/lib/containerd` read as 4.0K unprivileged — a 9.3G consumer was invisible; graceful fallback to plain `du` on hosts without passwordless sudo), and every background scan appends to a rolling snapshot file (`_record_snapshot`, keep `UA_DISK_HEALTH_SNAPSHOT_KEEP`=3, path `UA_DISK_HEALTH_SNAPSHOT_PATH` defaulting beside the activity DB) so a pressured finding also names **which directories grew most since the previous scan** (`_growth_since_previous` — top 3 above `UA_DISK_HEALTH_GROWTH_MIN_BYTES`=256M, in the message and `observed_value.growth_since_prev_scan`); dirs that just entered the top-N are excluded (no baseline). **The standing lesson: never write "X is the driver" into this module or this doc.** Every such claim has gone stale and then actively sent an operator to the wrong directory. Reclaim decisions come from the measured list, and a big dir may back a live service — check `grep -rl <path> /etc/systemd/system/*.service ~/.config/systemd/user/*.service` before deleting, and prefer deleting a regenerable `.venv` over its parent (some repos under `/home/ua/lrepos` have no git remote).
- **Registered git worktrees under `/opt/universal_agent/.worktrees` are a third measured category, automated 2026-07-24 (merged trees) and 2026-07-25 (open trees).** Re-measured 2026-07-25: **8,772,672,614 B (8.17 GiB) across 7 worktrees** — still larger than the live 6.9 GiB `.venv` and than `AGENT_RUN_WORKSPACES` (4,804 MiB), with 6.9 GiB of it a *single* per-worktree `.venv` (verified NOT hardlinked to `~/.cache/uv`, so removal frees the full amount). **The figure this bullet shipped with on 2026-07-24 — "15,700 MiB across 8 worktrees, 13.25 GiB of it two `.venv` trees" — went ~7 GiB stale within a day**, because an autonomous proactive-health run removed the 8th worktree (`codie-except-logging`, branch `codie/except-error-context`, MERGED) at 2026-07-25 00:06 UTC via `git worktree remove --force`. Both of those large `.venv` trees were behind **merged** PRs, so the implied claim that ~13 GiB sat behind *open* PRs was never true. Re-measure before acting on any number in this bullet. Nothing had ever reclaimed it: both reclaimers in the bullet above resolve their root from the VP-coder profile (`vp_coder_regenerable_reaper.py::_resolve_coder_workspace_root`, `vp_coder_workspace_pruner.py::_resolve_coder_workspace_root`), and `.worktrees/` is created by Claude Code sessions running as `ua`, entirely outside that root — `disk_usage_health.py` only *named* it in a runbook string telling a human to delete those `.venv` trees by hand. Now reclaimed by `vp_coder_workspace_pruner.py::prune_merged_worktrees`, a third tier on the existing weekly job (ten fail-closed guards, full description in [`project_docs/03_agents/01_vp_workers_and_delegation.md`](../03_agents/01_vp_workers_and_delegation.md) § "Workspace reaping"). **The trap to not re-introduce:** the obvious merged predicate, `git merge-base --is-ancestor <head> origin/main`, reclaims exactly zero bytes here — every PR squash-merges, so a merged branch's tip is never an ancestor of `main` (measured 2026-07-24: NOT-merged for all 8 worktree branches, including merged PRs #1482 and #1415). The GitHub PR state via `gh pr list --state merged` is the predicate that actually fires. Ships OFF (`UA_WORKTREE_PRUNE_ENABLED=0`) and dry-run (`UA_WORKTREE_PRUNE_DRY_RUN=1`); the unit gained `TimeoutStartSec=600` because the job is no longer pure-FS. Per the standing rule in the bullet above this is a measurement, not a claim that worktrees are *the* driver — re-measure before acting.

  **The open-PR half, added 2026-07-25:** `vp_coder_regenerable_reaper.py::reap_worktree_regenerable_artifacts` is a second root on the **daily** job that reaps `__pycache__` / `.pytest_cache` / `.ruff_cache` (and nothing else — a worktree `.venv` is NOT auto-regenerated and `node_modules` holds tracked content) from worktrees in the *same* allowlisted roots, explicitly **skipping merged trees** so it composes with the weekly prune instead of competing with it. Its daily unit therefore also gained `TimeoutStartSec=600`. **Honest reach: 73.3 MiB** (76,861,308 B) across the five open-PR worktrees — the rest of `.worktrees`' regenerable bytes are in merged trees the weekly job already reclaims more completely. Ships OFF (`UA_WORKTREE_REGENERABLE_REAP_ENABLED=0`) and dry-run (`UA_WORKTREE_REGENERABLE_REAP_DRY_RUN=1`) with a 72h quiescence window; full guard list in [`project_docs/03_agents/01_vp_workers_and_delegation.md`](../03_agents/01_vp_workers_and_delegation.md) § "Workspace reaping". **Shipped alongside it: a tracked-content guard for BOTH roots.** This repo tracks exactly one path under a regenerable name — `node_modules/.vite/vitest/<sha>/results.json` — and all 7 trees under `.worktrees` carry it (measured 2026-07-25), so reaping `node_modules` by name there would delete tracked source and dirty the tree, permanently failing guard 6 of the merged prune: the daily job would have mechanically disabled the weekly one for the same root. It is **not** a live bug on the existing VP-coder root — those 25 mission dirs sit on refs predating that file (`git ls-files -- node_modules` empty, no `node_modules/` present), so the guard is a no-op there today. A landmine disarmed before it was stepped on. If you are choosing where to spend risk here, the biggest reclaim by far is flipping `UA_WORKTREE_PRUNE_ENABLED=1` (dry-run first) on the weekly job — 6.94 GiB with guards already written, reviewed and tested.

  **The blind spot both halves shared, closed 2026-08-02 (VPS at 90.0%).** Scope came from `vp/worktree_utils.py::worktree_prune_roots`, which defaulted to `<repo>/.worktrees` alone — but the Claude Code harness puts its agent worktrees under `<repo>/.claude/worktrees`, and `UA_WORKTREE_PRUNE_ROOTS` is unset in prod. So both jobs *enumerated* `/opt/universal_agent/.claude/worktrees/fix-reflection-active-count-excludes-cron` and discarded it as out-of-scope while its `.venv` held **6.7 GiB** of torch + nvidia + triton (mtime 07-28) behind orphan commit `709db3ae` — on no remote branch, with no PR — and the daily reaper logged `Reaped 0 … 0.00 MiB` on each of its last three runs. Two fixes, both policy-only: the default root list now covers **both** directories, and the weekly prune distinguishes merged / PR-open / **no-PR-ever** (`gh_pr_exists`), aging out only the last of those and only behind four gates (7-day directory *and* content idleness, a 7-day-old branch tip, and `live_process_inside(deep=True)`). The daily reaper gained one narrow companion rule: a worktree `.venv` becomes reapable when — and only when — that same tree is idle, has nothing running inside it, and never had a PR.

  **Re-measure before treating worktrees as the driver.** As of 2026-08-02 several larger consumers sit in **no reaper's scope at all** and are report-only: `/home/ua/lrepos` **31G**, `/var/lib/containerd` **16G**, the `.claude-science` conda env **12G**, the huggingface cache **9.4G**. The worktree tiers are a measured category, not the whole alert.
