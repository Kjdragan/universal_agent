...
- **Markdown auto-renders (added 2026-06-22).** The shell transports files verbatim, so a published `.md` used to show as **raw source** in a browser. `cmd_publish` / `cmd_publish_dir` now detect markdown — a single `.md`, or a `--dir` containing any `.md` — and delegate to the Python renderers (`scratch_publish.py::publish_markdown_to_scratch` / `::publish_docset_to_scratch`), so you can hand the shell a raw `.md` (or a folder of them) and get a styled, cross-linked HTML page instead of source. The renderers call back into the script with the produced `.html`; an exported `UA_SCRATCH_RENDERING=1` guard makes that inner transport skip re-detection, so there is no recursion. If the UA venv/package isn't importable it falls back to raw transport with a stderr warning (a raw-but-delivered artifact beats a dropped one). HTML/images/PDFs/CSVs are unaffected — still served as-is.
...
- **Failure signature: phone/iOS gets `ERR_SSL_PROTOCOL_ERROR` ("sent an invalid response") on tailnet HTTPS links while desktops work (root-caused + fixed 2026-08-01).** Two stacked causes: (1) enabling **Funnel** on a node (e.g. uaonvps :10000) publishes PUBLIC DNS A-records for that hostname pointing at Tailscale's ingress relays — any client resolving via public DNS and hitting a NON-funneled port (443) gets a non-TLS reply; (2) **iOS browsers leak ts.net FQDN queries to public DNS** even when the Tailscale app shows "Using Tailscale DNS", on Wi-Fi and cellular alike. **Fix (tailnet-wide, one-time): admin console → DNS → add global nameservers (Cloudflare) → toggle "Override DNS servers" ON** (the toggle is greyed until a nameserver exists — verify by screenshot, not by "done"), then off/on the device's VPN. Debug rules that save hours: the only ground truth is a wire capture on the target (`sudo tcpdump -ni tailscale0 host <phone-ip>` while the operator taps — settings screens lie); the by-IP probe is a trap (tailscale serve rejects SNI-less connections with the SAME TLS internal-error alert a healthy client gets, so it proves nothing); always hand out DIRECTORY URLs ending in `/` (serve 301s explicit `…/index.html`). Full narrative: aod-build-language `lessons/LEDGER.md` (2026-08-01).
...
- **Durable per-project artifact archive (added 2026-06-21).** The live `/home/ua/ua_scratch/` store is flat (one dir per slug, no dates, no project grouping) and is the *only* copy. So after **every** successful publish, `scripts/publish_scratch.sh::archive_quiet` drops a second, permanent, dated copy into a **per-project archive root** and appends it to an ongoing index, via `scripts/scratch_archive.py::archive_artifact` (stdlib-only, runs locally on the publishing machine; best-effort — a failure never fails a publish). Because every publish path (the shell CLI *and* all four `scratch_publish.py` helpers) funnels through the one script, this is a single chokepoint with no per-caller wiring.
  - **Where it lands (`::resolve_archive_root`):** an explicit `UA_SCRATCH_ARCHIVE_ROOT` override always wins; on the VPS (autonomous runs) it's the permanent `/home/ua/ua_scratch_archive/`; on the desktop (interactive runs) it's a git-tracked `<repo>/scratch_archive/` *inside whatever repo you're working in* — so artifacts are organized per-project and committed with that project. Outside any git repo it falls back to `~/ua_scratch_archive/`. This makes the behavior **universal across projects** (the script is invoked by absolute path from any repo) while keeping each project's exhibits with that project.
  - **Layout:** `<root>/index.jsonl` (append-only ledger, source of truth), `<root>/INDEX.md` (human-readable, newest-first, git-friendly), `<root>/index.html` (browsable + searchable), and `<root>/<YYYY-MM-DD>/<HHMMSS>__<slug>__<name>` for files (a `<HHMMSS>__<slug>/` tree for docsets). Titles come from the `_artifact.json` sidecar when present, else the page `<title>` / markdown `# H1`, else the slug. Concurrent VPS publishes are serialized with an `fcntl` lock (`::_Lock`).
  - **Never pruned.** The archive is a **sibling** of `SCRATCH_ROOT`, not under it, so `prune_scratch.py` can never reach it; retention is indefinite by design. (The live `/scratch` store is itself unlimited-retention by default too — see the prior bullet.) The VPS archive is served read-only at `https://uaonvps.taildcc090.ts.net/scratch-archive/` (mount established by `::cmd_init`). Knobs: `UA_SCRATCH_ARCHIVE_ENABLED=0` disables archiving; `UA_SCRATCH_ARCHIVE_ROOT` overrides the root (tests pin it to a temp dir via `tests/conftest.py::_isolate_scratch_root`). In-repo, commits under `scratch_archive/**` are `paths-ignore`d by `deploy.yml` so archiving an exhibit never restarts prod.
- **HTML by default; render markdown only to view an existing `.md`.** Default to rich hand-authored HTML for exhibits/reports — graphics and interactivity carry more meaning, so HTML is the better choice for most deliverables. The markdown path (`scratch_publish.py::publish_markdown_to_scratch`, or the skill's markdown path) is specifically for when the operator wants to *see an existing markdown document displayed easily* — then publish it as rendered markdown (styled, light-mode, working TOC) rather than hand-converting it to HTML.
- **Stable id / refine-in-place.** All publish helpers take `artifact_id=` (`scratch_publish.py::_resolve_slug`): when set, the slug is exactly that id (no random suffix) so re-publishing overwrites the same dir/URL — one living exhibit you keep refining — and any review sidecars in that dir survive (they aren't in the freshly rendered tree). Omit it for one-off random-slug artifacts.
- **Two-way review (you ↔ Claude Code, no backend).** Every scratchpad-served HTML page carries a review toolbar (`scratch_publish.py::_REVIEW_TOOLBAR_JS` + `::_REVIEW_TOOLBAR_CSS`): rendered markdown/doc-set pages get it baked in by `::_html_page`, and **hand-authored HTML exhibits** (e.g. visual-explainer pages) get it injected by `::_inject_review_toolbar` — wired into `::publish_html_to_scratch` and the shell `.html` path (`publish_scratch.sh::_inject_toolbar`), so authored exhibits are markable too (the toolbar JS falls back to `<body>` when a page has no `<main>`). Injection is idempotent (a `<!-- scratch-review-toolbar -->` marker), and `::_run_publish_script` exports `UA_SCRATCH_RENDERING=1` on the transport hop so the shell never re-injects. The operator marks up the page — **highlight text → + Comment** (anchored to a stable `tag:nth-of-type` selector + occurrence index, so repeated text resolves unambiguously), **+ Element** to tap a diagram / image / table / chart (an element-target comment, touch-friendly), or **+ Note** for a general remark — then clicks Submit; the browser downloads `~/Downloads/scratch-comments-<slug>.json` (comments carry `target` / `selector` / `nth` / `elementLabel`) and copies a `[scratch-review <slug>]` prompt to the clipboard. An **open-time layout gate** (the JS `runAudit` pass) flags page-overflow / past-viewport / unrendered-Mermaid defects in a dismissible banner and writes them to `layout_audit[]` in the JSON (plus a one-line mention in the prompt), so render bugs surface even when the operator doesn't call them out — internal-scroll code blocks / tables are deliberately *not* flagged. Pasting the prompt into Claude Code is the whole continuation — **the responder is the interactive assistant, not the task hub / VP / project channels**. A future autonomous version (a project agent ingests comments and replies) and a tailnet write-back endpoint (for phone, no paste) are deliberately deferred.
