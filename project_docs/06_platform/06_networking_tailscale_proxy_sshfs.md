...
- **Two-way review (Phase 1, you ↔ Claude Code, no backend).** Pages rendered via `scratch_publish.py::_html_page` carry a review toolbar (`scratch_publish.py::_REVIEW_TOOLBAR_JS`): the operator marks up the page, clicks Submit, and the browser downloads `~/Downloads/scratch-comments-<slug>.json` and copies a `[scratch-review <slug>]` prompt to the clipboard. Pasting that into Claude Code is the whole continuation — **the responder is the interactive assistant, not the task hub / VP / project channels**. A future autonomous version (a project agent ingests comments and replies) and a tailnet write-back endpoint (`_comments.jsonl`, for phone) are deliberately deferred.
