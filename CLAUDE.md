...
- TL;DR: branch from `main` (per-task — Claude Code work uses `claude/<task>`, Codie work uses `codie/<task>`, operator work uses `kevin/<task>` or `feature/<task>`), push, run `/ship` (or `gh pr create --base main`). `pr-auto-merge.yml` auto-enables auto-merge for both `claude/*` and `codie/*` heads; CI runs; squash-merge fires `.github/workflows/deploy.yml`. `feature/latest2` retired 2026-05-13 — `main` is the home base. **Never push directly to `main`.**
