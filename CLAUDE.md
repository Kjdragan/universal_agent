...
- **Auto-merge → deploy uses `AUTO_MERGE_PAT` (added 2026-05-11 PM, PR #232).** `pr-auto-merge.yml` calls `gh pr merge --auto` with a fine-grained PAT (repo secret `AUTO_MERGE_PAT`, scopes: Contents+Pull-requests Read/Write) so the resulting squash-merge `push` to main is operator-attributed and fires `deploy.yml`. Earlier `GITHUB_TOKEN` version silently failed to trigger downstream workflows per GitHub's token-event suppression rule, causing three Hermes PRs (#228, #229, #230) to ship to main with deploy never firing. **Open follow-up:** `deploy.yml` has no `concurrency:` guard — simultaneous merges race on `/opt/universal_agent/.git/index.lock`. Recommended: add `concurrency: { group: deploy-production, cancel-in-progress: false }`. See [`docs/deployment/ci_cd_pipeline.md` § "Why a PAT" / "Concurrency caveat"](docs/deployment/ci_cd_pipeline.md#why-a-pat-2026-05-11-pr-232).
...
| **A new cron job, scheduled task, webhook handler, demo workspace consumer, or any new async unit of work** | **[`docs/03_Operations/129_Task_Hub_Observability_Protocol.md`](docs/03_Operations/129_Task_Hub_Observability_Protocol.md) — the canonical six-rule protocol (identity / claim ledger / run history / subprocess identity / protocol violation routing / standard recovery verbs). Use `ensure_cron_task_link` + `_open_run` + `classify_worker_exit` + `_close_run` + `park_task_for_protocol_violation`. The default for `_register_system_cron_job` is `skip_task_hub_link=False` — only opt out for pure no-state event handlers.** |
