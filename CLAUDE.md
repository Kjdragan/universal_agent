...
- **REQUIRED after every interactive (desktop) publish — commit the `scratch_archive/` entry.** The archiver only *writes* the durable copy + index; it does **not** commit. So an interactive publish is not "saved in the project" until you `git add scratch_archive/` and land it (branch → PR → auto-merge). `scratch_archive/**` is `paths-ignore`d in `deploy.yml`, so committing it never restarts prod. This is not optional and is the step most often skipped — `publish_scratch.sh` now prints a stderr reminder when the entry is left uncommitted; do not ignore it. (Commit only your own entry; don't sweep in another session's untracked artifacts.)
...
3. **Cody / all VPs — per-task CLI subprocesses (BOTH demo workspaces AND in-environment work)** → **ZAI (GLM) by default** as of 2026-06-07 (see `services/cody_mode.py::_HARDCODED_FALLBACK_MODE = "zai"` and every `vp/profiles.py` `VpProfile.inference_mode = "zai"` for CODIE/ATLAS/HOMER). This is a deliberate re-flip from the 2026-05-11 PM Anthropic-Max default: Anthropic began **API-billing the Claude-Code-via-Max SDK path**, so the autonomous per-task subprocesses now run on cheap ZAI inference like the in-process principals. Every Cody task carries a `cody_mode` resolved by `cody_mode.py::resolve_cody_mode` in this priority: (a) per-task override on `task_hub_items.cody_mode`, (b) per-VP DB setting, (c) global DB setting `cody_default_mode` (flippable from the dashboard tile), (d) `UA_CODY_DEFAULT_MODE` env var, (e) `VpProfile.inference_mode` for the resolving VP, (f) hardcoded `"zai"`. When a task is explicitly opted into `"anthropic"` (per-task / per-VP / env / dashboard), `vp/clients/claude_cli_client.py::_build_cli_env` strips every `ANTHROPIC_*` var from the spawned subprocess so OAuth wins, and force-enables `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`. That Anthropic opt-in applies to **all** Cody work — not just `/opt/ua_demos/<id>/` workspaces. Demo workspaces that need Anthropic add a second layer of defense via a vanilla `.claude/settings.json` in `src/universal_agent/templates/ua_demos_scaffold/`.

**Common past mistakes (don't repeat):**
- ❌ "Cody defaults to Anthropic Max for every task" — wrong since 2026-06-07. That was true during the 2026-05-11 PM → 2026-06-07 window, but Anthropic now API-bills the Claude-Code-via-Max SDK path, so Cody/all VPs default to **ZAI** (`_HARDCODED_FALLBACK_MODE = "zai"`, `VpProfile.inference_mode = "zai"`). Anthropic Max is now the **opt-in**: set it per-task (`cody_mode="anthropic"`), per-VP, via the dashboard tile, or `UA_CODY_DEFAULT_MODE=anthropic`.
- ❌ "Anthropic Max is the default for `/opt/ua_demos/` workspaces." Wrong — those default to ZAI too. But `_build_cli_env` does scrub `ANTHROPIC_*` for any Cody mission **explicitly** resolved to `cody_mode="anthropic"`, regardless of workspace location.
...
| Adding/changing an agent-execution timeout (turn kill, mission reap, no-progress) | Use the idle / no-progress watchdog — **never a hard wall-clock cap**. `timeout_policy.py::LivenessWatchdog` is the ONE shared policy (idle kill + tool-in-flight exemption + high absolute backstop), used by `execution_engine.py::ProcessTurnAdapter.execute`, `vp/clients/base.py::consume_adapter_events_with_idle_timeout`, and `vp/clients/claude_cli_client.py::_monitor_cli_output`. See [`project_docs/02_execution_core/01_gateway_sessions_execution.md`](project_docs/02_execution_core/01_gateway_sessions_execution.md) § "Liveness / no-progress timeout". A bare wall-clock cap killed live Simone work turns (2026-06-14). |
...
| Using/evaluating **GLM-5.2** (ZAI opus-tier candidate) or any `thinking` param on a ZAI model | [`project_docs/01_architecture/04_model_choice_and_resolution.md`](project_docs/01_architecture/04_model_choice_and_resolution.md) § "GLM-5.2 — validated opus-tier candidate". **GLM-5.1 had no thinking; GLM-5.2 defaults thinking ON** (`thinking:{type:enabled\|disabled\|auto}`). Wire id is bare `glm-5.2` (`[1m]` 400s); it mis-buckets to sonnet so pass `model_tier="opus"`; thinking is 10-24× the tokens so `{type:disabled}` for cheap crons + always `budget_tokens` when on. UA's parser already handles the thinking block (no empty-response risk on the Anthropic path). Eval harness: `python -m universal_agent.scripts.glm52_probe`. |
