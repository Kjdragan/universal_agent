...
|---|---|---|---|---|
| Phase A | Persistent CODIE session (no factories) | COMPLETE (READY_FOR_PHASE_B) | DraganCorp Core | CODIE can be dispatched/resumed/recovered with mission ledger + fallback path |
| Phase B | Shared-VPS multi-runtime VP lanes | READY_TO_START (DEFERRED_BY_CHOICE) | DraganCorp Core | Separate VP runtime service operational with quotas + callbacks |
...
- [x] A7: formalize Phase A closure and park Phase B as ready-but-deferred (explicitly preserving VP/factory continuity as an ongoing strategic program)
...
| 2026-02-16 | CODIE sustained-ops backlog | Added deferred investigation track for heartbeat + cron-driven self-maintenance hooks so CODIE monitoring can integrate with proactive heartbeat windows and scheduled job controls later | Preserves near-term VP delivery focus while explicitly capturing a high-value automation extension for future phases | Revisit during the next observability/automation expansion packet; define trigger model, guardrails, and rollout criteria |
| 2026-02-16 | CODIE routing reliability | Investigated delegation regression (coding probes not routed), found route gate still defaulted to feature-disabled posture when env flag was missing, changed routing to sustained default-on and expanded coding-intent markers to include shell/basics (`bash`, `shell`), then restarted gateway and reran 3 acceptance prompts | Restores core VP behavior (3/3 coding prompts delegated to CODIE) and aligns runtime defaults with sustained default-on policy while preserving explicit disable/shadow/fallback controls | Keep monitoring delegated-routing share; treat any drop below expected coding-intent coverage as a regression trigger |
| 2026-02-16 | Strict WS-E closure packet | Added VP session lifecycle event persistence (`vp_session_events`), wired CODIE lease heartbeat/release handling in gateway runtime flow, and extended CODIE metrics payload with lane-scoped recovery/orphan indicators + p50 latency | Closes remaining WS-E capability gaps (session lifecycle observability and lane-local recovery/orphan SLO fields) with additive, low-risk changes validated by targeted tests | Continue sustained default-on cadence and monitor new recovery/orphan fields for early drift detection |
| 2026-02-16 | Phase A closure decision | Formally marked Phase A complete and Phase B ready-to-start but deferred by operator choice; retained sustained CODIE monitoring and explicit rollback guardrails | Establishes clean handoff boundary: no immediate Phase B execution required, while preserving VP/factory roadmap continuity | Start Phase B only when intentionally re-prioritized; until then continue lightweight sustained observability cadence |
...
| D-IMP-014 | 2026-02-16 | Enforce sustained default-on semantics in routing by default (`coder_vp_enabled(default=True)`) and broaden coding-intent detection to include shell coding prompts (`bash`, `shell`) | Phase A is now in default-on posture; requiring an opt-in env flag caused real coding prompts to bypass CODIE after restart. This update makes CODIE the default coding lane while preserving explicit kill switches (`UA_DISABLE_CODER_VP`, shadow mode, force fallback). | If delegated routing causes undesired traffic on non-coding prompts or operational policy requires opt-in rollback |
| D-IMP-015 | 2026-02-16 | Close strict WS-E gap by adding dedicated CODIE session lifecycle event persistence and lane-local recovery/orphan metrics in `/api/v1/ops/metrics/coder-vp` | Ensures observability contract includes explicit session lifecycle signals (`vp.session.created/resumed/degraded`) plus CODIE-specific recovery/orphan indicators instead of relying only on global continuity metrics | If Phase B introduces centralized lane telemetry service and deprecates gateway-local aggregation |
| D-IMP-016 | 2026-02-16 | Declare Phase A complete and park Phase B as ready-but-deferred | All Phase A acceptance gates (A0-A7) are now satisfied with sustained default-on evidence, strict WS-E closure, and rollback controls still active; team can pause here without losing strategic continuity | If fallback/failure trend regresses, reopen Phase A stabilization before Phase B kickoff |
...
| 2026-02-16 | Sustained default-on must be encoded in runtime defaults, not only documented in rollout notes, otherwise a restart can silently revert behavior | Updated CODIE routing gate to default-on + expanded intent markers (`bash`, `shell`) and re-verified with 3/3 delegated acceptance prompts plus healthy mission metrics (`missions_considered=27`, `fallback=0`) |
| 2026-02-16 | WS-E can appear complete while still missing lane-local recovery/orphan visibility if observability is split across global and lane-specific endpoints | Added `vp_session_events` persistence and surfaced `recovery` + `session_health` blocks directly in CODIE metrics payload to make lane-level diagnosis self-contained |
