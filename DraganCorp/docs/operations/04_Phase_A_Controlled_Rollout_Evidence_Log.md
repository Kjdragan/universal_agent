...
| 2026-02-16T17:24:35Z | Sustained default-on delegation verification (post-fix) | Three coding acceptance prompts run as independent real sessions after routing fix + gateway restart | `scripts/coder_vp_rollout_capture.py --mode http --assessment-profile sustained --mission-limit 90 --event-limit 270` | 0.000 | 35.964s | SUSTAINED_DEFAULT_ON_HEALTHY | Post-fix acceptance probe observed 3/3 prompts with `routing=delegated_to_coder_vp`; metrics advanced to `missions_considered=27`, `missions_with_fallback=0`, `event_counts={vp.mission.dispatched:27,vp.mission.completed:27}`. |
