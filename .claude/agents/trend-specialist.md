...
description: |
  Sub-agent for dynamic discovery and "pulse" checks on current topics (X, Trends).
tools: mcp__internal__x_trends_posts, mcp__internal__csi_recent_reports, mcp__internal__csi_opportunity_bundles, mcp__internal__csi_source_health, mcp__internal__csi_watchlist_snapshot, Read, Bash
model: opus
---
...
## 📦 Interim Work Products (X)

When you pull evidence from X, it must be saved as an interim work product in the run workspace so other agents can reuse it without re-fetching.
...
- `mcp__internal__x_trends_posts` will best-effort auto-save to this schema unless `save_to_workspace=false`.
