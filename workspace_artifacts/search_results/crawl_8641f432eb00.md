...
`Ctrl+K`
mainv0.18.0v0.17.0v0.16.0v0.15.0v0.14.0v0.13.0v0.12.0v0.11.0v0.10.0v0.9.0v0.8.2v0.7.1v0.6.2 EN
...
to get started
Copy page
# [](https://huggingface.co/docs/peft/en/developer_guides/<#lora>) LoRA
...
## [](https://huggingface.co/docs/peft/en/developer_guides/<#initialization>) Initialization
...
### [](https://huggingface.co/docs/peft/en/developer_guides/<#pissa>) PiSSA
...
For detailed instruction on using CorDA, please follow these instructions.
### [](https://huggingface.co/docs/peft/en/developer_guides/<#olora>) OLoRA
...
For more advanced usage, please refer to our documentation.
### [](https://huggingface.co/docs/peft/en/developer_guides/<#eva>) EVA
EVA performs SVD on the input activations of each layer and uses the right-singular vectors to initialize LoRA weights. It is therefore a data-driven initialization scheme. Furthermore EVA adaptively allocates ranks across layers based on their “explained variance ratio” - a metric derived from the SVD analysis.
...
> For further instructions on using EVA, please refer to our documentation.
### [](https://huggingface.co/docs/peft/en/developer_guides/<#loftq>) LoftQ
#### [](https://huggingface.co/docs/peft/en/developer_guides/<#standard-approach>) Standard approach
...
#### [](https://huggingface.co/docs/peft/en/developer_guides/<#a-more-convenient-way>) A more convenient way
...
### [](https://huggingface.co/docs/peft/en/developer_guides/<#rank-stabilized-lora>) Rank-stabilized LoRA
...
### [](https://huggingface.co/docs/peft/en/developer_guides/<#activated-lora-alora>) Activated LoRA (aLoRA)
...
An example inference setup is at alora finetuning.
...
#### [](https://huggingface.co/docs/peft/en/developer_guides/<#using-and-reusing-cache-for-generation>) Using (and reusing) cache for generation
...
To demonstrate the above behaviors when using caching, we’re using DynamicCache from `transformers`. Care must be taken to ensure that adapted cache values are not mixed with base cache values. In particular, an extra step is required for sharing the cache when there is partial context overlap (pattern 2).
...
### [](https://huggingface.co/docs/peft/en/developer_guides/<#weight-decomposed-low-rank-adaptation-dora>) Weight-Decomposed Low-Rank Adaptation (DoRA)
...
### [](https://huggingface.co/docs/peft/en/developer_guides/<#qlora-style-training>) QLoRA-style training
...
### [](https://huggingface.co/docs/peft/en/developer_guides/<#memory-efficient-layer-replication-with-lora>) Memory efficient Layer Replication with LoRA
...
Fewshot-Metamath-OrcaVicuna-Mistral-10B is an example of a model trained using this method on Mistral-7B expanded to 10B. The adapter_config.json shows a sample LoRA adapter config applying this method for fine-tuning.
### [](https://huggingface.co/docs/peft/en/developer_guides/<#fine-grained-control-over-ranks-and-alpha-scaling>) Fine grained control over ranks and alpha (scaling)
By default, all layers targeted with LoRA will have the same rank `r` and the same `lora_alpha` (which determines the LoRA scaling), depending on what was specified in the LoraConfig. In some cases, however, you may want to indicate different values for different layers. This is possible by passing the `rank_pattern` and `alpha_pattern` arguments to LoraConfig. These arguments should be dictionaries with the key being the layer name and the value being the rank/alpha value. The keys can be regular expressions (regex). All LoRA layers that are not explicitly mentioned in `rank_pattern` and `alpha_pattern` will take the default `r` and `lora_alpha` values.
...
### [](https://huggingface.co/docs/peft/en/developer_guides/<#targeting-nnparameter-directly>) Targeting nn.Parameter directly
...
### [](https://huggingface.co/docs/peft/en/developer_guides/<#lora-optimized-lora>) LoRA+ optimized LoRA
LoRA training can be optimized using LoRA+, which uses different learning rates for the adapter matrices A and B, shown to increase finetuning speed by up to 2x and performance by 1-2%.
Copied
```
from peft import LoraConfig, get_peft_model
...
## [](https://huggingface.co/docs/peft/en/developer_guides/<#efficiently-train-tokens-alongside-lora>) Efficiently train tokens alongside LoRA
...
The token weights are part of your adapter state dict and saved alongside the LoRA weights. If we would have used full fine-tuning with `modules_to_save=['embed_tokens']` we would have stored the full embedding matrix in the checkpoint, leading to a much bigger file.
...
## [](https://huggingface.co/docs/peft/en/developer_guides/<#merge-lora-weights-into-the-base-model>) Merge LoRA weights into the base model
...
It is important to assign the returned model to a variable and use it, merge_and_unload() is not an in-place operation. If you need to keep a copy of the weights so you can unmerge the adapter later or delete and load different ones, you should use the merge_adapter() function instead. Now you have the option to use unmerge_adapter() to return the base model.
Copied
```
from transformers import AutoModelForCausalLM
from peft import PeftModel
...
## [](https://huggingface.co/docs/peft/en/developer_guides/<#load-adapters>) Load adapters
...
To return the base model, you could use unload() to unload all of the LoRA modules or delete_adapter() to delete the adapter entirely. unload() is not an in-place operation, remember to assign the returned model to a variable and use it.
Copied
```
# unload adapter
model = model.unload()
...
## [](https://huggingface.co/docs/peft/en/developer_guides/<#inference-with-different-lora-adapters-in-the-same-batch>) Inference with different LoRA adapters in the same batch
